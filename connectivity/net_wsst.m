function [sst,f]  = net_wsst(x,varargin)
%Wavelet Synchrosqueezed Transform
%   SST = net_wsst(X) returns the wavelet synchrosqueezed transform for the
...
%   samples in X.
%
%   [SST,F] = net_wsst(X) returns a vector of frequencies, F, in
%   cycles/sample corresponding to the rows of SST.
%
%   [...] = net_wsst(X,Fs) specifies the sampling frequency, Fs, in hertz
%   as a positive scalar. If you specify the sampling frequency, WSST
%   returns the frequencies in hertz.
%
%   [...] = net_wsst(X,Ts) uses the positive <a href="matlab:help duration">duration</a>,
%   Ts, to compute the scale-to-frequency conversion, F. Ts is the time
%   between samples of X. F has units of cycles/unit time where the unit of
%   time is the same time unit as the duration.
%
%   [...] = net_wsst(...,WAV) uses the analytic wavelet specified by WAV to
...
%
%   [...] = net_wsst(...,'VoicesPerOctave',NV) specifies the number of
%   voices per octave as a positive even integer between 10 and 48. The
%   number of scales is the product of the number of voices per octave and
%   the number of octaves. If unspecified, NV defaults to 32 voices per
%	octave. You can specify the 'VoicesPerOctave' name-value pair anywhere
%   in the input argument list after the signal X.
%
%   [...] = net_wsst(...,'ExtendSignal',EXTENDFLAG) specifies whether to
...
%
%   [...] = net_wsst(...,'FreqScale',SCALETYPE) specifies whether to
%   use a linear or an exponential frequency scale. SCALETYPE can be one
%   of the following options [ 'linear' | 'exp']. If unspecified,
%   SCALETYPE defaults to exp. You can specify the 'FreqScale' name-value
%   pair anywhere in the input argument list after the signal X.

%
%   net_wsst(...) with no output arguments plots the wavelet synchrosqueezed
...
if contains(params.frange,'linear')              % modified by JS, 05.05.17
    Nyquist = 1/(2*dt);
    Fund = 1/(nbSamp*dt);
    freq = linspace(Fund,Nyquist,na);
else
    log2Nyquist = log2(1/(2*dt));
    log2Fund = log2(1/(nbSamp*dt));
    freq = 2.^linspace(log2Fund,log2Nyquist,na);
end
Tx = 1/nv*sstalgo(cwtcfs,phasetf,params.thr,params.frange);
...
params.frange = 'exp';                           % modified by JS, 05.05.17
...
freqrange = find(strncmpi('FreqScale',varargin,1));             % modified by JS, 05.05.17

if any(freqrange)
    params.frange = lower(varargin{freqrange+1});
    
    if ~strcmp(params.frange,'linear') && ~strcmp(params.frange,'exp')
        error(message('Wavelet:FunctionInput:LinearOrExp'));
    end
    varargin(freqrange:freqrange+1) = [];
    if isempty(varargin)
        return;
    end
end
...
%------------------------------------------------------------------------
function Tx = sstalgo(cwtcfs,phasetf,gamma,freqRange)

M = size(cwtcfs,1);
N = size(cwtcfs,2);
if contains(freqRange,'linear')                  % modified by JS, 05.05.17
    Fund = 1/N;
    Nyquist = 1/2;
    iRow = real(1 + floor(M/(Nyquist-Fund)*(phasetf-Fund)));
else
    log2Fund = log2(1/N);
