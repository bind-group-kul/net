...
try
 ...
 % [tmp,tname] = fileparts(tempname);
  ...
  % % hdf5write(elecfile, 'region/gridlocs', int32(pos));  
  ...
  if ~ispc
    ...
    pp=zet_getpath('fns_tmp');  % changed by QL
    fprintf(efid,[pp filesep 'elecsfwd -img ' tempdir segfile ' -electrodes ' tempdir elecfile ' -data ', ...
                   tempdir datafile ' -contable ' tempdir confile ' -TOL ' num2str(tolerance) ' \n']);%2>&1 > /dev/null
  end
  fclose(efid);
  
  % run the shell instructions
  system(sprintf('chmod +x %s', exefile));   % changed by DM
  system(['/private/tmp/' exefile]);   % changed by DM
