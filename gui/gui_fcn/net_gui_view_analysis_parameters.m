function handles = net_gui_view_analysis_parameters(hObject, eventdata, handles)
...
gui_data = figure('Units','Normalized','NumberTitle','Off','Name','View analysis parameters',...
...
box = uiextras.VBox('Parent',gui_data,'Spacing',2*min(k),'Padding',5*min(k));    
...
handles.tabs = tabs;
tabnames = {'ACTIVITY ANALYSIS';'CONNECTIVITY ANALYSIS';'STATISTICS'};
% Show data from dataset excel file
sheets_list = {'activity_analysis';'connectivity_analysis';'statistical_analysis'};
for s = 1:3
...
%     'CellSelectionCallback',@select_data);
    ...
    tabData=uix.ScrollingPanel('Parent',tabs,'Padding',5*min(k));
    tabData.Tag = sheet_name;

    box_param = uiextras.HBox('Parent',tabData,'Spacing',min(k),'Padding',5*min(k));
    ...
    set(box_param,'Sizes',[200*k(2) 140*k(2) 55*k(2) 10*k(2)])
...
%     'Widths',500,... % widths of contents, in pixels and/or weights
...
if ~exist(handles.analysis_filename,'file')
    copyfile(handles.template_analysis_path,handles.analysis_filename);
end

sheets_list = {'activity_analysis';'connectivity_analysis';'statistical_analysis'};
% copyfile(handles.template_analysis_path,fullfile(pwd,'test.xlsx'));
%copyfile(fullfile(pwd,'prova.xlsx'),fullfile(pwd,'test.xlsx'));
for s = 1:3
    sheet_name = char(sheets_list(s));
    ...
    writetable(new_data,handles.analysis_filename,'Sheet',s,'Range','B2','WriteVariableNames',false); % handles.analysis_filename
...
% copyfile(fullfile(pwd,'test.xlsx'),handles.analysis_filename);
% delete(fullfile(pwd,'test.xlsx'));
