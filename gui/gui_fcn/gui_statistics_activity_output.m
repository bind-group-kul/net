function gui_statistics_activity_output(handles)

k = handles.k;
fontsize = handles.fontsize;
fontname = handles.fontname;

fig_name = 'Group statistics - Activity';
...
fig_output = figure('Units','Normalized','Position',[0.01 0.66 0.25 0.74],...
...
mainbox = uiextras.VBox('Parent',fig_output,'Padding',10*min(k));
    ...
    uicontrol('Parent',mainbox,'Style','Text','String','Choose statistics:',...
        'FontName',fontname,'FontSize',fontsize);    

    stat_panel = uix.Panel('Parent',mainbox,'Title','','FontName',fontname,'FontSize',fontsize);

        hbox = uiextras.HBox('Parent',stat_panel);
                handles.group_choose_stat = uibuttongroup('Parent',hbox,'BorderType','none',...
                    'SelectionChangedFcn',@update_menu);
                    uicontrol('Parent',handles.group_choose_stat,'Style','radiobutton','String','FFX',...
                        'Position',[135 25 100 30],'FontName',fontname,'FontSize',fontsize)
                    uicontrol('Parent',handles.group_choose_stat,'Style','radiobutton','String','RFX',...
                        'Position',[135 0 100 30],'FontName',fontname,'FontSize',fontsize)
    ...
    'FontName',fontname,'FontSize',fontsize,'Padding',5*min(k));
        ...
        hbox = uiextras.HBox('Parent',erd_ers_panel,'Spacing',10*min(k),'Padding',5*min(k));
            ...
            handles.box_ers_erd_buttons = uiextras.VBox('Parent',hbox,'Padding',5*min(k));
                
                uicontrol('Parent',handles.box_ers_erd_buttons,'Style','text','String','Time-frequency map',...
                    'FontName',fontname,'FontSize',fontsize);
                
                handles.select_tfchan = uicontrol('Parent',handles.box_ers_erd_buttons,'Style','popupmenu','String','Choose channel',...
                    'FontName',fontname,'FontSize',fontsize,'Tag','tfchan','Callback',@show_tfmap);
                
                handles.select_tfmap = uicontrol('Parent',handles.box_ers_erd_buttons,'Style','popupmenu','String','Choose ROI',...
                    'FontName',fontname,'FontSize',fontsize,'Callback',@show_tfmap);

                uicontrol('Parent',handles.box_ers_erd_buttons,'Style','text','String','Topoplot',...
                    'FontName',fontname,'FontSize',fontsize);
                
                handles.select_topoplot = uicontrol('Parent',handles.box_ers_erd_buttons,'Style','popupmenu','String','Choose condition/freq',...
                    'FontName',fontname,'FontSize',fontsize,'Callback',@show_ers_erd_topoplot);
                
                uicontrol('Parent',handles.box_ers_erd_buttons,'Style','text','String','Source map',...
                    'FontName',fontname,'FontSize',fontsize);

                handles.select_map = uicontrol('Parent',handles.box_ers_erd_buttons,'Style','popupmenu','String','Choose map',...
                    'FontName',fontname,'FontSize',fontsize,'Callback',@show_img);
            set(handles.box_ers_erd_buttons,'Sizes',[25*k(2) 25*k(2) 35*k(2) 25*k(2) 35*k(2) 25*k(2) 25*k(2)]) % 40*k(2) 30*k(2)])

            uiextras.Empty('Parent',hbox);

        set(hbox,'Sizes',[-1 200*k(2) -1])

set(mainbox,'Sizes',[30*k(2) 55*k(2) 20*k(2) 30*k(2) 250*k(2) 10*k(2) 250*k(2)])

% Update menu
erp_signal_dir = [handles.outdir filesep 'group' filesep 'eeg_signal' filesep 'erp_results'];
erp_source_dir = [handles.outdir filesep 'group' filesep 'eeg_source' filesep 'erp_results'];
ers_erd_signal_dir = [handles.outdir filesep 'group' filesep 'eeg_signal' filesep 'ers_erd_results'];
ers_erd_source_dir = [handles.outdir filesep 'group' filesep 'eeg_source' filesep 'ers_erd_results'];

suffix = 'ffx';
handles.suffix = suffix;

if ~exist([erp_signal_dir filesep 'erp_timecourses_sensor_' suffix '.mat'],'file')
    set(handles.select_erpchan,'Enable','Off')
    set(handles.select_erptopoplot,'Enable','Off')
else
    erp_filename = [erp_signal_dir filesep 'erp_timecourses_sensor_' suffix '.mat'];
    load(erp_filename);
    chan_list = erp(1).label;
    s = {'Choose channel'};
...
if ~exist([erp_source_dir filesep 'erp_timecourses_roi_' suffix '.mat'],'file')
    set(handles.select_erproi,'Enable','Off')
else
    erp_filename = [erp_source_dir filesep 'erp_timecourses_roi_' suffix '.mat'];
    load(erp_filename);
    roi_list = erp(1).label;
    s = {'Choose ROI'};
...
if ~isdir([erp_source_dir filesep 'mni'])
    set(handles.select_erpmap,'Enable','Off')
else
    search_dir = [erp_source_dir filesep 'mni' filesep '*_' suffix '.nii'];
...
if ~exist([ers_erd_signal_dir filesep 'ers_erd_sensor_' suffix '.mat'],'file')
    set(handles.select_tfchan,'Enable','Off')
    set(handles.select_topoplot,'Enable','Off')
else
    ers_erd_filename = [ers_erd_signal_dir filesep 'ers_erd_sensor_' suffix '.mat'];
    load(ers_erd_filename);
    chan_list = ers_erd(1).label;
    s = {'Choose channel'};
    for m = 1:length(chan_list)
        s = [s; chan_list(m)];
    end
...
if ~exist([ers_erd_source_dir filesep 'ers_erd_roi_' suffix '.mat'],'file')
    set(handles.select_tfmap,'Enable','off')
else
    ers_erd_filename = [ers_erd_source_dir filesep 'ers_erd_roi_' suffix '.mat'];
    load(ers_erd_filename);
    load(options.ers_erd.seed_file);
    roi_list = {seed_info.label};
    s = {'Choose ROI'};
    for m = 1:length(roi_list)
        s = [s; roi_list(m)];
    end
...
if ~isdir([ers_erd_source_dir filesep 'mni'])
    set(handles.select_map,'Enable','Off')
else
    search_dir = [ers_erd_source_dir filesep 'mni' filesep '*_' suffix '.nii'];
...
guidata(gcbo,handles)
                    ...
                    % --- start update_menu
function update_menu(hObject, eventdata, handles)
handles = guidata(gcbo);

    set(allchild(handles.box_erp_buttons),'Enable','On','Value',1)
    set(allchild(handles.box_ers_erd_buttons),'Enable','On','Value',1)
    ...
    suffix = lower(get(handles.group_choose_stat.SelectedObject,'String'));
    handles.suffix = suffix;
    ...
    if ~exist([ers_erd_signal_dir filesep 'ers_erd_sensor_' suffix '.mat'],'file')
        ...
        chan_list = ers_erd.ers_erd(1).label; %changed by JS 06.2023
        s = {'Choose channel'};
        for m = 1:length(chan_list)
            s = [s; chan_list(m)];
        end
...
%         roi_list = {ers_erd.seed_info.label};
        load(options.ers_erd.seed_file); %modified by JS 06.2023
        roi_list = {seed_info.label};
        s = {'Choose ROI'};
        for m = 1:length(roi_list)
            s = [s; roi_list(m)];
...
handles = guidata(gcbo);
    ...
    if idx > 1
        ...
        fig_img = figure('Units','Normalized','Position',[0.34 0.6 0.5 0.5],...
            'NumberTitle','Off','Name',['Group - ERP time-courses - ' str_selection char(label) ' - ' upper(suffix)]);
        fig_img.set('Menubar','figure');
        fig_img.set('Pointer','watch');
        drawnow

        if strcmpi(hObject.Tag,'erpchan')
            load([handles.outdir filesep 'group' filesep 'eeg_signal' filesep 'erp_results' filesep 'erp_timecourses_sensor_' suffix '.mat']);
            erp_data = erp;
        else
            load([handles.outdir filesep 'group' filesep 'eeg_source' filesep 'erp_results' filesep 'erp_timecourses_roi_' suffix '.mat']);
            erp_data = erp;
        end
...
%         setappdata(groot,'erp',erp)
        suffix = handles.suffix;
        erp_filename = [handles.outdir filesep 'group' filesep 'eeg_signal' filesep 'erp_results' filesep 'erp_timecourses_sensor_' suffix '.mat'];
        topoplot_file = load(erp_filename);
        topoplot_data = topoplot_file.erp;
    
        lat_val = get(select_topoplot,'String');
        lat_val = lat_val(latency+1);
        
        fig_img = figure('Units','Normalized','Position',[0.34 0.6 0.5 0.25],...
            'NumberTitle','Off','Name',['Group - ERP topoplots - Latency: ' char(lat_val) ' ms - ' upper(suffix)]);
        fig_img.set('Menubar','figure');
        
        template_elecpos = [handles.net_path filesep 'template' filesep 'electrode_position' filesep handles.parameters.pos_convert.template '.sfp'];

        maxval = 0; minval = 0;
        for idx = 1:size(topoplot_data,2)
            [~,topoval] = topoplot(topoplot_data(idx).erp_tc(:,latency)',template_elecpos);
...
handles = guidata(gcbo);
    ...
    if condition > 0
        suffix = handles.suffix;
        ers_erd_filename = [handles.outdir filesep 'group' filesep 'eeg_signal' filesep 'ers_erd_results' filesep 'ers_erd_sensor_' suffix '.mat'];
        topoplot_file = load(ers_erd_filename);
        topoplot_data = topoplot_file.ers_erd;
        ...
        fig_img = figure('Units','Normalized',...
            'NumberTitle','Off','Name',['Group - ERS/ERD topoplots - ' str_condition ' - Frequency: ' str_frequency ' Hz - ' upper(suffix)]);
        fig_img.set('Menubar','figure');
...
%         end
        ...
        template_elecpos = [handles.net_path filesep 'template' filesep 'electrode_position' filesep handles.parameters.pos_convert.template '.sfp'];
        topoplot(topomap, template_elecpos, 'whitebk', 'on', 'maplimits',maplimit);
        
        %load('cm_blue_white_red.mat');
        %colormap(gca, blue_white_red);
        colormap(bipolar(256,maplimit(1),maplimit(2)))
        bar = colorbar('Position',[0.88 0.168 0.032 0.7]); c.FontSize = 0.9*handles.fontsize;
        ylabel(bar, 'ERD/ERS (%)','FontSize',handles.fontsize)
...
handles = guidata(gcbo);
    ...
    if idx_tfmap > 1
        ...
        if strcmpi(hObject.Tag,'tfchan')
            ers_erd_filename = [handles.outdir filesep 'group' filesep 'eeg_signal' filesep 'ers_erd_results' filesep 'ers_erd_sensor_' suffix '.mat'];
            load(ers_erd_filename);
            ers_erd_data = ers_erd;
            str_title = 'Channel';
        else
            ers_erd_filename = [handles.outdir filesep 'group' filesep 'eeg_source' filesep 'ers_erd_results' filesep 'ers_erd_roi_' suffix '.mat'];
            load(ers_erd_filename);
            ers_erd_data = ers_erd;
            str_title = 'ROI';
        end
        ...
        fig_img = figure('Units','Normalized','Position',[0.34 0.6 0.7 0.77],...
            'NumberTitle','Off','Name',['Group - ERS/ERD time-frequency maps - ' str_title ': ' roi_name ' - ' upper(suffix)]);
        fig_img.set('Menubar','figure');
        ...
        for i = 1:num_conditions
            ...
            set(gca, 'YDir', 'normal', 'FontSize',0.8*handles.fontsize)
            xlabel('Time (s)','FontSize',handles.fontsize)
            ylabel('Frequency (Hz)','FontSize',handles.fontsize)
            name_condition = ers_erd_data(i).condition_name;
            name_condition = strrep(name_condition,'_',' ');
            title(name_condition,'FontSize',handles.fontsize)
            c = colorbar('Location','southoutside');
            c.Label.String = 'ERD/ERS (%)'; c.FontSize = 0.8*handles.fontsize;
...
%         c.Label.String = 'ERD/ERS (%)'; c.FontSize = 0.9*handles.fontsize;
        clear ers_erd ers_erd_filename
    end
...
handles = guidata(gcbo);
    ...
    if idx_map > 1
        ...
        title = [str_title ' map: ' char(seedimg_filename{1}(1:end-4)) ' - Template MRI - ' upper(suffix)];
