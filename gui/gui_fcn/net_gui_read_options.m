function [options]=net_gui_read_options(pathx,flag)

switch flag
    
    case 'prepro'
        
        xls_convert      = xls2struct(pathx,1); % 'conversion');
        xls_head         = xls2struct(pathx,2); % 'head_modelling');
        xls_preprocessing= xls2struct(pathx,3); % 'signal_processing');
        xls_source       = xls2struct(pathx,4); % 'source_localization');
    ...
    case 'analysis'
        ...
        xls_activity     = xls2struct(pathx,1); % 'activity_analysis');
        xls_connectivity = xls2struct(pathx,2); % 'connectivity_analysis');
        xls_stats        = xls2struct(pathx,3); % 'statistical_analysis');
