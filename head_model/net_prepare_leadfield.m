function [grid, cfg] = net_prepare_leadfield(cfg,data)
...
if strcmpi(cfg.headmodel.type, 'gfdm')
        cfg.solver.tol  = 1e-10;
        cfg.solver.maxit  = 700;     
        cfg.vol = cfg.headmodel;
        
        % Projecting electrodes to the Volume, ensuring that every single 
        % electrode voxel is surrounded by Non Zero data
        cfg.elec = gfdm_prepare_elecs( cfg );
        
        % Precalculate and store Leadfields
        cfg.gmask = 'gm';
        lead = gfdm_precalculate_leads( cfg );
%         save('lead_fdm_1mm.mat', '-v7.3', 'lead');
%         load('lead_fdm_1mm.mat');
%         load('prv_lead_fdm.mat');
        cfg.grid = ft_convert_units(cfg.grid, 'mm'); % cfg.subject_grid->cfg.grid % Gaia
        grid = gfdm_calculate_pots(lead, cfg);
else


% collect and preprocess the electrodes/gradiometer and head model
[headmodel, sens, cfg] = net_make_headmodel(cfg, data);
...
tmpcfg           = keepfields(cfg, {'grid','headshape', 'symmetry', 'smooth', 'threshold', 'spheremesh', 'inwardshift'});
...
if ft_voltype(headmodel, 'openmeeg')
  ...
  while ~all(ok)
    ...
    batch  = begdip:enddip;
    try
      lf = net_compute_leadfield(grid.pos(insideindx(batch),:), sens, headmodel);
  ...
  for i=1:length(insideindx)
    ...
    grid.leadfield{thisindx} = net_compute_leadfield(grid.pos(thisindx,:), sens, headmodel);
