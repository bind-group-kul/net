function masksegm = net_segment_mrtim(data_anat,tpm,res)
...
hi=histc(Ior(Ior>0.01*max(Ior(:))),int);

n = 1; % n = 3; %%%%% 28.01.20 !!!!!!!
hi_avg = hi;%tsmovavg(hi,'s',n,1); %%%%% 28.01.20 !!!!!
...
% preliminary masks
wm_b = Iwm.*(data_tpm_b>0.01); % 0.1
wm_c = Iwm.*(data_tpm_c>0.01);
wm_bs = Iwm.*(data_tpm_bs>0.01);
...
eyes = data_tpm>0.7; % Gaia 07.04.20 *** 0.9 ULTIMO
CC=bwconncomp(eyes);
...
mask_er = imerode(mask,strel('sphere',1));
...
% tissue probability map (NET)
% img_filename = '/Users/u0114283/Documents/PhD/MR tissues/new_segm/mr_data/rc9anatomy_prepro.nii';
% Vtpm=spm_vol(img_filename);
% data_tpm_muscle=spm_read_vols(Vtpm);
...
muscle(find(masksegm==12)) = 0;
%{
masktot = (masksegm>0 & masksegm<9 | masksegm==11); % <7
masktot = imfill(masktot,'holes');      % Gaia 14.11.19
muscle = imcomplement(masktot).*(data_tpm_muscle>0.7);

% no overlap compacta
idxmuscle = intersect(find(muscle),find(masksegm==8));
muscle(idxmuscle) = 0;
%}

segment_new(:,:,:,10) = muscle;
% masksegm(find(muscle)) = 9; % Gaia 09.12.19
...
% tissue probability map (NET)
% img_filename = '/Users/u0114283/Documents/PhD/MR tissues/new_segm/mr_data/rc10anatomy_prepro.nii';
% Vtpm=spm_vol(img_filename);
% data_tpm=spm_read_vols(Vtpm);

data_tpm_fat = tpm(:,:,:,10);
fat = data_tpm_fat>0.5;
fat(find(masksegm)) = 0;
...
%{
masktot = (masksegm>0 & masksegm<10 | masksegm==11);
masktot = imfill(masktot,'holes');  % Gaia 14.11.19
fat = imcomplement(masktot).*(data_tpm>0.3);
%}
...
% border spong --> comp   % Gaia 12.11.19
maskspong = (masksegm==7);
maskspong_dil = imdilate(maskspong,strel('sphere',2));
idxcomp = find(maskspong_dil-maskspong);
masksegm(idxcomp)=8;
%}
...
% else
%     masksegm(find(mask_gap)) = -1;
% end
masksegm(find(eyes))=11;    % Gaia 19.11.19

% close skin
masktot = (masksegm~=0); % masksegm>0
masktot = imfill(masktot,'holes');
...
maskskin = (masksegm==12);
maskskin = imfill(maskskin,'holes');
masksegm = masksegm .* maskskin;

%{
%% Save segmentation image
% disp('Saving tissue segmentation...')

odir = output_folder;
V2.fname = [odir filesep 'anatomy_prepro_segm.nii'];
spm_write_vol(V2,masksegm);

% Save single tissue masks
if job.segtiss.tiss_mask == 1
    folder_tiss = [odir filesep 'tissue_masks'];
    if ~exist(folder_tiss,'dir')
        mkdir([odir filesep 'tissue_masks'])
    end
    V2.fname = [odir filesep 'tissue_masks/bGM.nii'];
    spm_write_vol(V2,masksegm==1);
    V2.fname = [odir filesep 'tissue_masks/cGM.nii'];
    spm_write_vol(V2,masksegm==2);
    V2.fname = [odir filesep 'tissue_masks/bWM.nii'];
    spm_write_vol(V2,masksegm==3);
    V2.fname = [odir filesep 'tissue_masks/cWM.nii'];
    spm_write_vol(V2,masksegm==4);
    V2.fname = [odir filesep 'tissue_masks/brainstem.nii'];
    spm_write_vol(V2,masksegm==5);
    V2.fname = [odir filesep 'tissue_masks/CSF.nii'];
    spm_write_vol(V2,masksegm==6);
    V2.fname = [odir filesep 'tissue_masks/spongiosa.nii'];
    spm_write_vol(V2,masksegm==7);
    V2.fname = [odir filesep 'tissue_masks/compacta.nii'];
    spm_write_vol(V2,masksegm==8);
    V2.fname = [odir filesep 'tissue_masks/muscle.nii'];
    spm_write_vol(V2,masksegm==9);
    V2.fname = [odir filesep 'tissue_masks/fat.nii'];
    spm_write_vol(V2,masksegm==10);
    V2.fname = [odir filesep 'tissue_masks/eyes.nii'];
    spm_write_vol(V2,masksegm==11);
    V2.fname = [odir filesep 'tissue_masks/skin.nii'];
    spm_write_vol(V2,masksegm==12);
    if job.segtiss.gapfill == 0
       V2.fname = [odir filesep 'tissue_masks/gap.nii'];
        spm_write_vol(V2,masksegm==-1); 
    end
end
%}
