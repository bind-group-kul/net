function [ ] = net_cardiac_correction_skew( processedeeg_filename, options)
%NET_REMOVE_ECG
%
    ...
    if strcmp(options.enable, 'on')
        ...
        %% 2.5 Cardiac artifact removal: non-referential with windowed kurtosis
        bad_ics_skew = [];
        if(strcmp(options.skewness_enable, 'on'))
        ...
        if(strcmp(options.reference_enable, 'on'))
            ref_channel = selectchannels(D,'ECG');
            if(~isempty(ref_channel))
                ...
                disp('Could not find ECG channel as reference for cardiac artifact removal. Please check the conversion page of xls file!');
            end
        end
        
        %% 2.7 define bad ICs, and remove artifacts
        bad_ics = unique([bad_ics_skew, bad_ics_corr]);
        ...
        if(strcmp(test, 'on'))
            ...
            IC_file = [preprocess_path, filesep, 'IC_S4_cardiac.mat'];
            mixing_matrix = dewhitening(:,1:n_pcs)*mixing;
            unmixing_matrix = unmixing*whitening(1:n_pcs, :);
            save(IC_file, 'mixing_matrix', 'unmixing_matrix', 'IC', 'dewhitening', 'whitening', 'mixing', 'n_pcs', 'latent', 'PC', 'unmixing', 'bad_ics','Fs', 'bad_ics_skew', 'bad_ics_corr');
        end
