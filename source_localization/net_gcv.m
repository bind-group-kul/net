...
if (strncmp(method,'Tikh',4) | strncmp(method,'tikh',4))
  ...
  if (find_min)
      GInv = 1.01*max(G) - G;
      [Minima,MinIdx] = findpeaks(GInv);
      [mina,minb]=min(Minima);
      
      if isempty(Minima)
          [minG,minGi] = min(G); % Initial guess.
          reg_min = fminbnd('gcvfun',...
              reg_param(min(minGi+1,npoints)),reg_param(max(minGi-1,1)),... %%reg_param(min(minGi+1,npoints)),reg_param(max(minGi-1,1)),...
              optimset('Display','off'),s2,beta(1:p),delta0,m-n); % Minimizer.
      else
          reg_min=reg_param(MinIdx(minb));
      end
