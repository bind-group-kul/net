...
elseif ismeg
  ...
  switch ft_voltype(vol)
    ...
    case 'singleshell'
      ...
      [center,radius] = fitsphere(vol.bnd.pos);
      ...
      if size(sens.coilpos,1)>0 && ~isfield(vol, 'forwpar')
        ...
        vol.forwpar = meg_ini([s*vol.bnd.pos vol.bnd.nrm], s*center', order, [s*sens.coilpos sens.coilori]);
        vol.forwpar.scale = s;
      end
...
elseif iseeg
  ...
  switch ft_voltype(vol)
    ...
    case {'bem', 'dipoli', 'asa', 'bemcp', 'openmeeg'}
      ...
      if ~isfield(vol, 'tra') && (isfield(vol, 'mat') && ~isempty(vol.mat))
        ...
        if size(vol.mat,1)~=size(vol.mat,2) && size(vol.mat,1)==length(sens.elecpos)
          ...
          [el, prj] = project_elec(sens.elecpos, vol.bnd(vol.skin_surface).pos, vol.bnd(vol.skin_surface).tri); 
          tra       = transfer_elec(vol.bnd(vol.skin_surface).pos, vol.bnd(vol.skin_surface).tri, el);

            
          % replace the original electrode positions by the projected positions
          sens.elecpos = prj;
          
          if size(vol.mat,1)==size(vol.bnd(vol.skin_surface).pos,1)
            ...
            for i=1:length(vol.bnd)
              if i==vol.skin_surface
                ...
                interp = [interp, zeros(size(el,1), size(vol.bnd(i).pos,1))];
              end
            end
          end
          ...
          if strcmp(ft_voltype(vol), 'openmeeg')
            ...
            nb_points_external_surface = size(vol.bnd(vol.skin_surface).pos,1);
    ...
    case 'fns'
      if isfield(vol,'bnd')
        [el, prj] = project_elec(sens.elecpos, vol.bnd.pos, vol.bnd.tri);
        sens.tra = transfer_elec(vol.bnd.pos, vol.bnd.tri, el);
        % replace the original electrode positions by the projected positions
        sens.elecpos = prj;
      end
    ...
    case 'simbio'
      ...
      %if not(isfield(vol,'stiff'))  % added by DM 29/03/17
      %if not(isfield(vol,'transfer')) 
