function output = H_methods_PDC ( data, config )
...
%     the PDC and DTF algorithms. NFFT refers only to the positive part of
...
if ~isfield ( config, 'towindow' ), config.towindow = false;     end % added by JS, 20.02.17
...
% Windows the data.
if config.towindow                                         % added by JS, 20.02.17
...
if DTF_on, output.DTF.data = zeros ( channels, channels, config.nfft, windows ); end
...
%     output.crit = var(A,[],3);                                                              %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if DTF_on, Bf = zeros ( size ( Af ) ); end          % modified by JS, 20.02.17
    Af_norm = zeros ( size ( Af ) );
    if DTF_on, Bf_norm = zeros ( size ( Af ) ); end     % modified by JS, 20.02.17
...
% Save frequencies used in fft.
fft_f = config.window.fs*(0:(L/2))/L; % added by JS, 20.02.17
output.fftFreq = fft_f(1:config.nfft)';
