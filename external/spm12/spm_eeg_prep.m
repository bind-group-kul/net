    ...
    case 'loadeegsens'
        ...
        switch S.source
            ...
            case 'locfile'
                ...
                % Remove Vref: add by JS 17.01.2020
                vref = strfind(lower(label), 'ref'); vrefind = -1;
                for z = 1:numel(vref), if ~isempty(vref{z}), vrefind = z; end,end
                if (vrefind > 0), label(vrefind) = []; clear z vref vrefind; end
                
                % This handles FIL Polhemus case and other possible cases
                % when no proper labels are available.
                for z = 1:numel(label), lab = label{z}; label{z} = lab(~isspace(lab)); end
                if isempty(intersect(label, elec.label))
%                     ind = str2num(strvcat(elec.label)); this works only with numeric labels -> see new version below
                    ind = 1:numel(elec.label); % mod by JS 17.01.2020
                    if length(ind) == length(label)
                        elec.label(ind,1) = label(ind);
                    else
                        error('To read sensor positions without labels the numbers of sensors and EEG channels should match.');
                    end
                end
