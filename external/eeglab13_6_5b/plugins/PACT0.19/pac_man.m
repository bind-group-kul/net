...
if any(allChanStdRatio<0.1)
    ...
    for n = 1:length(badChanIdx)
        disp(sprintf('Channel %.0f have abnormally low SD. Remove this channel from analysis.', badChanIdx(n)))
    end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
    case 1
        ...
        for n = 1:EEG.nbchan
            EEG.pac.hfoIndex{n,1} = find(logicalHasMap(n,:));
        end
...
%%%%%%%%%%%%%%%%%%%%%%%%%% 
    case 2
        ...
        for n = 1:EEG.nbchan
            EEG.pac.hfoIndex{n,1} = find(globalHFOmask(n,:));
        end
    ...
    case 3 % handpicked
        ...
        for n = 1:EEG.nbchan
            ...
            if any(tmpHfoListIndex)
                ...
                EEG.pac.hfoIndex{n,1} = tmpHfoListWin;
            end
        end
        
        % create non-empty channel list
        nonEmptyChannelList = false(1,length(EEG.pac.hfoIndex));
        for n = 1:length(EEG.pac.hfoIndex)
             nonEmptyChannelList(n,1) = ~isempty(EEG.pac.hfoIndex{n,1});
        end
end
...
% EEG.pac.phaseBinValues                = zeros(EEG.nbchan, numPhaseBin+1);
...
for n = 1:EEG.nbchan
    ...
    % update waitbar    
    waitbar(n/EEG.nbchan, waitBar, ['Processing Channel ' num2str(n)]);
    
    hfoIndex = EEG.pac.hfoIndex{n,1};
    if isempty(hfoIndex)
        disp(['Channel ' num2str(n) ' has no HFO marker.'])
    else
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%% compute HFO-indexed data  %%%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % find their Angle
        analyAmp   = abs(  EEG.pac.analyticEEG(n,:));
        analyPhase = angle(EEG.pac.analyticEEG(n,:));
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % vector length mean
        EEG.pac.vectLength(n,1)    = circ_r(hfoPhase');
        % vector length variance 
        EEG.pac.vectLengthVar(n,1) = circ_var(hfoPhase');    
        % angle mean
        ...
        % store results
        EEG.pac.angleMean(n,1) = tmpAngle;
        EEG.pac.angleStd(n,1)  = tmpAngleStd;
                
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%% compute Modulation Index of this HFO %%%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        tmpMI            = abs(mean(EEG.pac.analyticEEG(n,hfoIndex)));
        EEG.pac.mi(n,1)  = tmpMI; 
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        skip = ceil(length(hfoPhase).*rand(numSurro*3,1)); 
        skip(skip>maxskip) = [];
        skip(skip<minskip) = [];
        skip = skip(1:numSurro,1);
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        for s = 1:numSurro
            tmpSurroPhase   = [analyPhase(skip(s):end) analyPhase(1:skip(s)-1)]; % randomize phase instead of amp- consider circular shifts
            surroPhase(:,s) = tmpSurroPhase(hfoIndex)';
        end
        ...
        %%% p-value %%%
        %%%%%%%%%%%%%%% 
        EEG.pac.uncorrected.MIpval(n,1) = stat_surrogate_pvals(SurroMI, tmpMI, 'upper');
        
        %%%%%%%%%%%%%%%%%%%%%%%%%%%
        %%% confidence interval %%%
        %%%%%%%%%%%%%%%%%%%%%%%%%%%        
        sortSurroMI = sort(SurroMI);
        confInt95   = ceil(length(SurroMI)*0.95);
        confInt99   = ceil(length(SurroMI)*0.99);
        EEG.pac.moduInd95CI(n,1) = sortSurroMI(confInt95);
        EEG.pac.moduInd99CI(n,1) = sortSurroMI(confInt99);
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        % store bin values
        EEG.pac.phaseBinValues(n,:) = binValue'; 
        
        % uniform distribution statistics (3 circular statistics)
        EEG.pac.uncorrected.phaseRayleighPval(n,1) = circ_rtest(binValue);   % Rayleigh test
        EEG.pac.uncorrected.phaseOmniTestPval(n,1) = circ_otest(binValue);   % omnibus or Hodges-Ajne test test
        EEG.pac.uncorrected.phaseRaoSpacePval(n,1) = circ_raotest(binValue); % Rao's spacing test
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        for s = 1:numPhaseBin
            phaseSortedAmp(s,1)   = mean(hfoAmp(binIndex==s));
            % phaseSortedAmp(s,1)   = median(hfoAmp(binIndex==s));
            phaseSortedAmpSe(s,1) = std(hfoAmp(binIndex==s))/sqrt(length(find(binIndex==s)));
        end

        % sort from 0 to 2pi
        phaseSortOrder                = [numPhaseBin/2+1:numPhaseBin 1:numPhaseBin/2];
        EEG.pac.phaseProbability{n,1} = phaseProbability(phaseSortOrder);
        EEG.pac.phaseSortedAmp{n,1}   = phaseSortedAmp(phaseSortOrder);
        EEG.pac.phaseSortedAmpSe{n,1} = phaseSortedAmpSe(phaseSortOrder);
        ...
        EEG.pac.uncorrected.ampChi2GofPval(n,1) = p;
        ...
        EEG.pac.uncorrected.ampKstestPval(n,1) = p;     
        
        % keep record of max bin value
        EEG.pac.histBinMax(n,1) = max(phaseProbability);
    end
end
...
%%%%%%%%%%%%%%%%%%
for n = 1:EEG.nbchan
    [EEG.pac.uncorrected.phaseWtsnWillPval(n,1),~] = circ_wwtest(EEG.pac.phaseBinValues(n,:), EEG.pac.angleMean);
end
