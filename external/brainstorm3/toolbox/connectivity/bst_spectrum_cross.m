function [spectra, freq] = bst_spectrum_cross(X, Y, inputs)
% BST_SPECTRUM  Main method for calculating the cross spectrum of a set of signals.
%
% Inputs:
%   X           - MX signals, each of length N
%                 [X: MX x N matrix]
%   Y           - MY signals, each of length N
%                 [Y: MY x N matrix] (default: if left empty, Y is set to X for auto spectrum)
...
switch method
  ...
  case {'periodogram', 'welch'} 
    ...
    [spectra, freq] = bst_spectrum_periodogram(X, Y, freq, Fs, segmentLength, overlap, windowName, taperPct, ci, maxfreqres);
...
%     end
%     
%     [transfers, noiseVariance] = bst_mvar(data, order, nTrials, true);
...
function [spectra, freq, lower, upper] = bst_spectrum_periodogram(X, Y, freq, Fs, segmentLength, overlap, windowName, taperPct, ci, maxfreqres)
% BST_PERIODOGRAM   Calculate two-sided periodogram
%
% Inputs:
%   X               - MX signals - one in each column - each of length N
%                     [X: MX x N matrix]
%   Y               - MY signals, - one in each column - each of length N
%                     [Y: MY x N matrix]
%   inputs          - Structure of parameters:
%   freq            - Frequencies of interest
%                     [F: NF x 1 vector]
...
%% ===== EVERY SEGMENT =====
spectra = zeros(nX, nY, length(freqInitial));
for idxSegment = 1:nSegments
    
    % Frequency domain spectrum after smoothing and tapering
    fourierX = fft(bst_bsxfun(@times, X(:, segmentIndices(1,idxSegment):segmentIndices(2,idxSegment)), smoother'), nFFT, 2);
    fourierY = fft(bst_bsxfun(@times, Y(:, segmentIndices(1,idxSegment):segmentIndices(2,idxSegment)), smoother'), nFFT, 2);
    
    % SLOWEST
    % fourierRow = ...
    %   repmat(reshape(fourierX, [nSignals 1 nFFT/2]), [1 nSignals 1]);
    % fourierCol = ...
    %   repmat(reshape(fourierY, [1 nSignals nFFT/2]), [nSignals 1 1]);
    % spectrum = fourierRow .* conj(fourierCol);
    
    % NEXT SLOWEST
%     for stop = 1:nX
%       for start = 1:nY
%         spectrum(stop, start, :) = fourierX(stop,1:(nFFT/2)) .* conj(fourierY(start,1:(nFFT/2)));
%       end
%     end
...
% interp + FFT for frequency vector length 2^N is faster than wronskian
...
if ~isempty(freq) % Interpolate to desired frequencies
    spectra = reshape(interp1(freqInitial, reshape(spectra, [], nFFT/2).', freq).', [nX nY length(freq)]);
