function [transfer, err, p, transferFull, errFull, fpe, aic, hq, sc, residual] = bst_mvar(signals, order, nTrials, flagFPE)
...
%   transferFull       - If flagFPE = true, list of transfer estimations for order 1, 2, ..., inputs.order
%   errFull            - If flagFPE = true, list of error covariances for order 1, 2, ..., inputs.order
%   aic                - Akaike Information Criterion measurement [2]
...
%
% Call:
%   [transfer, err, p, transferFull, errFull, fpe, aic, hq, sc] = bst_mvar(sources, order, nTrials, algorithm, flagFPE)
...
if flagFPE
  ...
  errFull(:,:,1)  = bst_correlation(signals, signals, struct()); % Start: order zero AR -> error is covariance of sources
  ...
  for p = 1:order
    ...
    residual = present - transferFull(:, 1:(nSignals*p), p+1)*past(1:(nSignals*p), :);
    errFull(:, :, p+1) = bst_correlation(residual, residual, struct()); % Error in fit
  ...
  p = find(diff(fpe) ./ min(diff(fpe)) > 0.05, 1, 'last') + 1;
%   p = find(abs((fpe - min(fpe)) ./ fpe) > 0.1, 1, 'last') + 1;
  if p > order
    p = order;
  end
...
%% Optimal order known
  ...
  err = bst_correlation(residual, residual, struct()); % Error in fit
