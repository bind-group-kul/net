...
if isdir(sTemplate.FilePath)
    ...
    if ~isempty(strfind(sTemplate.FilePath, 'http://')) || ~isempty(strfind(sTemplate.FilePath, 'https://')) || ~isempty(strfind(sTemplate.FilePath, 'ftp://'))
        ...
        % Download file
        downloadManager = java_create('org.brainstorm.file.BstDownload', 'Ljava.lang.String;Ljava.lang.String;Ljava.lang.String;)', sTemplate.FilePath, ZipFile, 'Download template');
        ...
        if (downloadManager.getResult() ~= 1)
            bst_error(['Impossible to download template:' 10 downloadManager.getMessage()], 'Download error', 0);
            return
        end
...
% Close process bar
bst_progress('stop');
% Delete unzipped tempalte folder
if isDeleteDir
    file_delete(templateDir, 1);
end
