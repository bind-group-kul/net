...
elseif (length(studyFiles) > 1)
    ...
    try
        ...
        warning('Brainstorm:InvalidFile', 'Cannot open file ''%s'' : ignoring study', bst_fullfile(studiesDir, studySubDir, studyFile));
        studyMat = [];
    end
end
...
isChannelFile = ~cellfun(@(c)isempty([findstr(lower(c),'_channel'), findstr(lower(c),'channel_')]), {dirFiles.name});
        ...
        switch(fileType)
            ...
            case 'noisecov'
                ...
                if ~isempty(noisecovInfo)
                    sStudy(1).NoiseCov = noisecovInfo;
                end
    ...
    function sInfo = io_getChannelInfo(relativeFilename)
        ...
        if file_exist(bst_fullfile(studiesDir, relativeFilename))
            try
                ...
                if isempty(findstr(sInfo(1).Comment, ')'))
