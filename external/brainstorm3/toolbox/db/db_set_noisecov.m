function OutputFiles = db_set_noisecov(iSrcStudy, Target)
% DB_SET_NOISECOV: Apply a noise covariance node to other studies
%
% USAGE:  OutputFiles = db_set_noisecov(iSrcStudy, iDestStudies)    : Apply to the target studies
...
if isempty(sSrcStudy.NoiseCov) || isempty(sSrcStudy.NoiseCov.FileName)
    error('No noise covariance available in the source srudy.');
end
...
% ===== READ SOURCE DATA =====
% Load noisecov file
SrcNoiseCovMat = load(file_fullpath(sSrcStudy.NoiseCov.FileName));
% Load channel file
...
for i = 1:length(iDestChanStudies)
    ...
    if ~isempty(iDestChan)
        ...
        NewFile = import_noisecov(iDestStudy, DestNoiseCovMat);
