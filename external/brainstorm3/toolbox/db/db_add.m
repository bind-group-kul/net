...
if isstruct(InputFile)
    ...
    if strcmpi(fileType, 'timefreq')
        if ismember(lower(sMat.Method), {'corr', 'cohere', 'granger', 'plv', 'plvt'})
            if ismember(sMat.Options.ProcessName, {'process_corr1n', 'process_cohere1n', 'process_granger1n', 'process_plv1n'})
...
if ismember(fileType, {'subjectimage', 'channel', 'noisecov'})
    % Get file to replace
    delfile = '';
    switch(fileType)
        case 'subjectimage'
