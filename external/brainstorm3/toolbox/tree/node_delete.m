...
%% ===== DATABASE =====
    case {'subjectdb', 'studydbsubj', 'studydbcond'}
        ...
        if (iProtocol >= 1)
            db_delete_protocol(iProtocol, isUserConfirm);
        end
    ...
    case {'condition', 'rawcondition'}
        ...
        if isUserConfirm
            if strcmpi(nodeType{1}, 'rawcondition')
                questStr = ['<BR><BR><FONT color="#CC0000">Warning: Removing links to raw files does not acually remove the raw files.<BR>' ...
...
%% ===== ANATOMY =====
    case 'anatomy'
        ...
        % Delete file
        if file_delete(FullFilesList, ~isUserConfirm)
...
%% ===== DATA FILE / DATA LIST =====
    case {'data', 'datalist', 'rawdata'}
        ...
        if isequal(iStudies_data, -10) || isequal(iStudies_results, -10) || isequal(iStudies_timefreq, -10)
        ...
        for i = 1:length(iStudies_data)
            ...
            if strcmpi(nodeType{1}, 'rawdata')
                BinFile = strrep(DataFile, '.mat', '.bin');
                if file_exist(BinFile)
                    FullFilesList{end+1} = BinFile;
                end
            end
...
%% ===== RESULT FILE =====
    case {'results', 'kernel'}
        ...
        if (file_delete(FullFilesList, ~isUserConfirm) == 1)
            ...
            % Get unique list of studies
            uniqueStudies = unique([iStudies, iStudies_timefreq]);
            for i = 1:length(uniqueStudies)
                iStudy = uniqueStudies(i);
...
%% ===== MATRIX =====
    case 'matrix'
        ...
        if file_delete(FullFilesList, ~isUserConfirm)
            ...
            for i=1:length(iUniqueStudy)
                iStudy = iUniqueStudy(i);
                iMatrix = iSubItem(iItem == iStudy);
                iTimefreqDel = iTimefreq(iStudies_timefreq == iStudy);
                sStudy = bst_get('Study', iStudy);
                % Remove file description from database
                sStudy.Matrix(iMatrix) = [];
                sStudy.Timefreq(iTimefreqDel) = [];
                % Study was modified
                bst_set('Study', iStudy, sStudy);
            end
