...
if (strcmpi(FileType, 'anatomy') && ~isfield(value, 'Cube')) || ...
   ...
   (ismember(lower(FileType), {'pdata', 'presults', 'ptimefreq'}) && ~isfield(value, 'tmap')) || ...
...
% MAT-file format
if ismember(lower(FileType), {'data', 'rawdata', 'results', 'kernel', 'pdata', 'presults', 'timefreq', 'ptimefreq', 'spectrum', 'matrix'})
    matVersion = 'v6';
else
    matVersion = 'v7';
end
