...
%     - targetNodeType : {'data', 'results', 'pdata', 'presults','ptimefreq', 'raw', 'rawcondition', 'matrix', 'any'}
...
if strcmpi(targetNodeType, 'any') && (length(bstNodes) >= 1)
    switch (lower(nodeTypes{1}))
        ...
        case 'pdata'
            targetNodeType = 'pdata';
        case 'presults'
            targetNodeType = 'presults';
        case 'ptimefreq'
        ...
        switch (lower(nodeTypes{iNode}))
            ...
            case {'studydbsubj', 'studydbcond'}
                ...
                for iSubj = 1:length(uniqueSubjects)
                    ...
                    [sStudies, iStudies] = bst_get('StudyWithSubject', SubjectFile);
            ...
            case 'studysubject'
                ...
                if (nodeStudies(iNode) == 0)
                    ...
                    [sStudies, iStudies] = bst_get('StudyWithSubject', nodeFileNames{iNode});
                    iTargetStudies = [iTargetStudies, iStudies];
...
function isPure = isPureKernel(sResults)
    isPure = cellfun(@isempty, {sResults.DataFile}) & ...
             ~cellfun(@(c)isempty(findstr(c, 'KERNEL')), {sResults.FileName});   
end
