...
% Empty input
if isempty(FileName)
    disp('FULLPATH> Input filename is empty');
    return;
...
switch lower(FileType)
    case {'brainstormsubject', 'subject', 'subjectimage', 'anatomy', 'scalp', 'outerskull', 'innerskull', 'cortex', 'other', 'tess'}
        ...
        isAnatomy = 1;
    case {'brainstormstudy', 'study', 'studysubject', 'condition', 'rawcondition', 'channel', 'headmodel', 'data', 'rawdata', 'results', 'kernel', 'pdata', 'presults', 'noisecov', 'dipoles', 'timefreq', 'spectrum', 'ptimefreq', 'matrix', 'proj'}
        if ~file_exist(FileName)
