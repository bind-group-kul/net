function Projector = import_ssp(ChannelFile, SspFiles, ApplyToData, SaveChannelFile)
% IMPORT_SSP: Reads SSP projectors from a list of FIF files, add it to the channel file and to all the dependant data.
%
% USAGE:  import_ssp(ChannelFile, SspFiles, ApplyToData, SaveChannelFile)
%         import_ssp(ChannelFile, Projector, ApplyToData, SaveChannelFile)
%         import_ssp(ChannelFile)
...
% =============================================================================@
%
% Authors: Lucie Charles, Francois Tadel, 2010-2012
...
if ~isempty(Projector) && ~isstruct(Projector)
    Projector = process_ssp('ConvertOldFormat', Projector);
end
% Check if there is actually something to apply
if isempty(Projector)
    disp('SSP> Error: No new projectors to apply.');
    bst_progress('stop');
    return;
end
...
if isempty(ApplyToData) % || (any(isRaw) && ~all(isRaw))
    res = java_dialog('question', ['Would you like to apply all those projections ' 10 ...
...
if (nChanProj ~= nChanFile)
    error('There are %d channels, but %d rows in the projector.', nChanFile, nChanProj);
end
...
%% ===== APPLY TO RAW FILES =====
iFileRaw = find(isRaw);
for i = 1:length(iFileRaw)
    iFile = iFileRaw(i);
    % Load file
    DataFile = file_fullpath(DataFiles{iFile});
    DataMat = in_bst_data(DataFile);
    % Add projector to the channel file
    if ~isfield(DataMat.F.channelmat, 'Projector') || isempty(DataMat.F.channelmat.Projector)
        DataMat.F.channelmat.Projector = Projector;
    else
        DataMat.F.channelmat.Projector = [DataMat.F.channelmat.Projector, Projector];
    end
    % Keep only time bounds
...
if ApplyToData
    ...
    P = process_ssp('BuildProjector', Projector, [0 1 2]);
    if ~isempty(P)
        iNonRaw = find(~isRaw);
