function sFile = in_fopen_fif(DataFile, ImportOptions)
% IN_FOPEN_FIF: Open a FIF file, and get all the data and channel information.
%
% USAGE:  sFile = in_fopen_fif(DataFile, ImportOptions)
%         sFile = in_fopen_fif(DataFile)
%
% INPUT: 
...
% catch
%     warning('Could not find measurement data.');
%     info.chs = [];
...
elseif (nEpochs == 0)
    ...
    if iscell(ImportOptions.EventsMode) || ~strcmpi(ImportOptions.EventsMode, 'ignore')
        sFile.events = fif_read_events(sFile, ImportOptions);
