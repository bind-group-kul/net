...
% =============================================================================@
%
% Authors: Francois Tadel, Elizabeth Bock 2012

%% ===== GET EVENTS =====
evt = sFile.events;
nEventGroups = length(evt);
% Must be a real link to a CTF file
if ~isfield(sFile, 'channelmat') || ~isfield(sFile.channelmat, 'Channel') || ~isfield(sFile.channelmat.Channel, 'Type')
...
iVideo = find(strcmpi({sFile.channelmat.Channel(:).Type}, 'Video'));
if isempty(iVideo)
    error('No video time channel in this file');
end
[F, TimeVector] = in_fread(sFile, [], sFile.prop.samples, iVideo);
