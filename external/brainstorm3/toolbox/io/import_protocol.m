...
% EXPORT_PROTOCOL: Import a protocol from a zip file.
% 
...
if isempty(ZipFile)
    % Select folder file
    ZipFile = java_getfile('open', 'Load protocol...', '', 'single', 'files', ...
                           {{'.zip'}, 'Brainstorm protocol (*.zip)', 'protocol'}, 1);
    if isempty(ZipFile)
        return
    end
end
...
if any(strcmpi({GlobalData.DataBase.ProtocolInfo.Comment}, ProtocolName))
    bst_error(sprintf(['Protocol ''%s'' already exists in database.' 10 ...
                       'Please rename zip file to import it in database.'], ProtocolName), 'Load protocol', 0);     
    return;
end
...
if file_exist(ProtocolDir)
    bst_error(sprintf(['Folder ''%s'' already exists in database.' 10 ...
                       'Please rename zip file to import it in database.'], ProtocolDir), 'Load protocol', 0);     
    return;
end
