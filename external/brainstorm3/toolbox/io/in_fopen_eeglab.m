function sFile = in_fopen_eeglab(DataFile, ImportOptions)
...
if ~hdr.isRaw
    ...
    if (length(listParam) >= 1) && ImportOptions.DisplayMessages
            ...
            for iTrial = 1:nEpochs
                ...
                while ~isCondFound && (iCond <= length(ParamSelected))
                    ...
                    while (isOk && (iParam <= length(paramsList)))
                        ...
                        if iscell(epoch.(['event', paramsList{iParam}]))
...
sFile.prop.nAvg    = 1;
% Channel file
sFile.channelmat = in_channel_eeglab_set(hdr, ImportOptions.ChannelAlign >= 1);
sFile.header = hdr;
