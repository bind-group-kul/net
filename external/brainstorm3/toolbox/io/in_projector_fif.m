...
%     % Compute the resulting projector
%     [Projector.Status] = deal(true);
%     P1 = process_ssp('BuildProjector', Projector, [0,1,2]);
%     if isempty(P1)
%         disp('No valid projector, can''t go any further...');
%         return
%     end
%     % Get a matrix of all the projectors that will be applied to the data for the forward modeling
%     [projs.active] = deal(true);
%     P2 = mne_make_projector(projs, tmp, []);
%     % Difference
%     disp(['MAX DIFFERENCE: ' num2str(max(abs(P1(:)-P2(:))))]);
% end
