function NewFiles = import_data(DataFiles, FileFormat, iStudyInit, iSubjectInit, ImportOptions)
...
%         NewFiles = import_data(sFile, FileFormat, iStudyInit, iSubjectInit, ImportOptions)
%
% INPUT:
...
if (nargin < 4) || isempty(iSubjectInit)
    iSubjectInit = 0;
end
if (nargin < 3) || isempty(iStudyInit) || (iStudyInit == 0)
    iStudyInit = 0;
else
...
for iFile = 1:length(DataFiles)  
    ...
    % Load file
    if ~isempty(sFile)
        [ImportedDataMat, ChannelMat, nTime, ImportOptions] = in_data(sFile, FileFormat, ImportOptions, nbCall);
        ...
        [ImportedDataMat, ChannelMat, nTime, ImportOptions] = in_data(DataFile, FileFormat, ImportOptions, nbCall);
    end
    ...
    elseif (iStudyInit == 0) && (iSubjectInit ~= 0) && ~isnan(iSubjectInit)
        ...
        [sStudies, iStudies] = bst_get('StudyWithCondition', bst_fullfile(sSubject.Name, DataFile_base));
        % If does not exist yet: Create the default study
        if isempty(iStudies)
            iStudies = db_add_condition(sSubject.Name, DataFile_base);
            if isempty(iStudies)
