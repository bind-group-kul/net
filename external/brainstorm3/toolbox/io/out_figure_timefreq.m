...
if (nargin < 2) || isempty(OutputFile)   
    ...
    OutputFile = bst_fullfile(LastUsedDirs.ExportData, [initBase, '_extract', initExt]);
    OutputFile = file_unique(OutputFile);
    % Get filename where to store the filename
    [OutputFile, TfFormat] = java_getfile('save', 'Save time-frequency map', OutputFile, 'single', 'files', ...
            {{'_timefreq'}, 'Brainstorm structure (*timefreq*.mat)', 'BST'; ...
             {'.txt'},      'ASCII file (*.txt)',                 'ASCII'}, 1);
...
elseif strcmpi(TfFormat, 'BST')
    bst_save(OutputFile, OutputMat, 'v6');

% ===== EXPORT TO TEXT FILE =====
else
    % Save file
    dlmwrite(OutputFile, TF);
end
