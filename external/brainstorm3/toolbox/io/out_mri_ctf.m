function out_mri_ctf( BstFile, OutputFile )
% OUT_MRI_CTF: Exports a Brainstorm MRI in CTF .mri file format, Version 4.1
%
% USAGE:  out_mri_ctf( BstFile, OutputFile )
%
% INPUT: 
...
% ===== CONVERT FIDUCIALS =====
nas = [MRI.SCS.NAS(1) ./ MRI.Voxsize(1), ...
       size(MRI.Cube,2) - MRI.SCS.NAS(2) ./ MRI.Voxsize(2), ...
       size(MRI.Cube,3) - MRI.SCS.NAS(3) ./ MRI.Voxsize(3)];
lpa = [MRI.SCS.LPA(1) ./ MRI.Voxsize(1), ...
       size(MRI.Cube,2) - MRI.SCS.LPA(2) ./ MRI.Voxsize(2), ...
       size(MRI.Cube,3) - MRI.SCS.LPA(3) ./ MRI.Voxsize(3)];
rpa = [MRI.SCS.RPA(1) ./ MRI.Voxsize(1), ...
       size(MRI.Cube,2) - MRI.SCS.RPA(2) ./ MRI.Voxsize(2), ...
       size(MRI.Cube,3) - MRI.SCS.RPA(3) ./ MRI.Voxsize(3)];
center = (lpa + rpa) ./ 2;
transf = [[MRI.SCS.R; center], [0;0;0;1]];
...
ind2 = 1:min(256,s(2));
ind3 = 1:min(256,s(3));
Vol(ind1,ind2,ind3) = MRI.Cube(ind1,ind2,ind3);
% Re-orient
...
write_tag('_CTFMRI_MMPERPIXEL', 10, sprintf('%3.10f\\%3.10f\\%3.10f', MRI.Voxsize));
