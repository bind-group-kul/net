...
function Add(strType, sProcess, sInputs, strMsg)
    ...
    if isempty(sInputs)
        FileNames = [];
    elseif ischar(sInputs)
        FileNames = {sInputs};
    elseif iscell(sInputs)
        FileNames = sInputs;
    ...
    try
        switch (SnapType)
            ...
            case 'noisecov'
                ...
                % Load file
                NoiseCovMat = load(file_fullpath(sChanStudy.NoiseCov.FileName));
                % Display as image
            ...
            case 'ssp'
                ...
                for iProj = 1:length(ChannelMat.Projector)
                    ...
                    for iComp = 1:length(iSelComp)
                        ...
                        iAllChan = find(comp);
                        AllMod = intersect(unique({ChannelMat.Channel(iAllChan).Type}), {'EEG','MEG','MEG GRAD','MEG MAG','ECOG','SEEG'});
                        if all(ismember(AllMod, {'MEG GRAD','MEG MAG'}))
                        ...
                        for iMod = 1:length(AllMod)
                            ...
                            hFig(end+1) = view_topography(FileName, AllMod{iMod}, '2DSensorCap', comp(iChan), [], 1);
...
function ReportFile = Save(sInputs, ReportFile)
    ...
    if (nargin < 2) || isempty(ReportFile)
        ...
        % Full file name
        ReportFile = bst_fullfile(reportsDir, ['report' strProtocol strTime '.mat']);
    end
    % Add stop entry
    Add('stop', [], sInputs, []);
    % Save 
...
function jReport = Open(ReportFile, isFullReport)
    ...
    if ~isempty(iStart)
        ...
        if iscell(Reports{iStart,3})
            FilesInit = strrep(Reports{iStart,3}, '\', '/');
        elseif isstruct(Reports{iStart,3})
            FilesInit = strrep({Reports{iStart,3}.FileName}, '\', '/');
        end
    else
        ProtocolInfo = [];
    end
    ...
    FilesFinal = {};
    if ~isempty(iStop)
        if iscell(Reports{iStop,3})
            FilesFinal = strrep(Reports{iStop,3}, '\', '/');
        elseif isstruct(Reports{iStop,3})
            FilesFinal = strrep({Reports{iStop,3}.FileName}, '\', '/');
        end
    end
    % Process tags
    if ~isempty(iProcess)
        ...
        for i = 1:length(iProcess)
            if iscell(Reports{iStop,3})
                FilesInterm = cat(2, FilesInterm, strrep(Reports{iProcess(i),3}, '\', '/'));
            elseif isstruct(Reports{iStop,3})
                FilesInterm = cat(2, FilesInterm, strrep({Reports{iProcess(i),3}.FileName}, '\', '/'));
            end
        end
    ...
    if ~isFullReport
        ...
        elseif (length(iErrors) == 1) && isempty(iWarnings)
            ...
            FileNames   = Reports{iErrors(1),3};
            strMsg      = Reports{iErrors(1),4};
            % Convert input structures to filenames
    ...
    %if ~isempty(iMessages)
        ...
        for i = 1:length(iMessages)
            ...
            elseif (length(FileNames) == 1)
                strFile = ListFiles(FileNames, ProtocolInfo, 1);
            else
    ...
    if ~isempty(FilesInit) && ~isequal(FilesInit, {[]})
        html = [html ListFiles(FilesInit, ProtocolInfo, 0)];
    else
    ...
    if ~isempty(FilesInterm)
        ...
        if ~isempty(FilesInterm) && ~isequal(FilesInterm, {[]})
            html = [html ListFiles(FilesInterm, ProtocolInfo, 0)];
        else
    ...
    if ~isempty(FilesFinal) && ~isequal(FilesFinal, FilesInit)
        ...
        if ~isempty(FilesFinal) && ~isequal(FilesFinal, {[]})
            html = [html ListFiles(FilesFinal, ProtocolInfo, 0)];
        else
    ...
    if ~isempty(iImages)
        ...
        for i = 1:length(iImages)
            ...
            html = [html, ListFiles(FileNames, ProtocolInfo, 1)];
...
%% ===== LIST FILES =====
function str = ListFiles(FileNames, ProtocolInfo, isWrap)
    str = [];
...
function ClearHistory()
    ...
    if isdir(reportsDir)
        ...
        for iFile = 1:length(allReports)
            delete(bst_fullfile(reportsDir, allReports(iFile).name));
        end
    end
end
...
function Recall(target)
    ...
    if (sProcesses(1).nInputs == 1)
        ...
        if (isempty(sCurFiles) && ~isempty(FileNames)) || ~isequal({sCurFiles.FileName}, FileNames)
            % Reset files lists
        ...
        error('Not supported yet for processes with two inputs.');
    end
end
