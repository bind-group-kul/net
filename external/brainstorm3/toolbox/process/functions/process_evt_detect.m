...
% =============================================================================@
%
% Authors: Elizabeth Bock, Francois Tadel, 2011-2013
...
function sProcess = GetDescription() %#ok<DEFNU>
    ...
    % Examples: EOG, ECG
    sProcess.options.example.Comment = ['<HTML><BR>&nbsp; Examples:<BR>' ...
...
function evt = Compute(F, TimeVector, OPTIONS, Fmask)
    ...
    if ~isempty(OPTIONS.bandpass)
        F = process_bandpass('Compute', F, TimeVector, OPTIONS.bandpass(1), OPTIONS.bandpass(2));
    end
    % Absolute value
    Fabs = abs(F);
    % Standard deviation
