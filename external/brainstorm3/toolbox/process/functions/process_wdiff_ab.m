...
function sProcess = GetDescription() %#ok<DEFNU>
    ...
    sProcess.Comment     = 'Weighted difference';
    sProcess.FileTag     = [];
    ...
    sProcess.options.ttest_label.Comment    = ['<HTML>Corrects the difference of signal amplitude between two averages<BR>' ...
...
function sOutput = Run(sProcess, sInputsA, sInputsB) %#ok<DEFNU>
    % Difference
    sOutput = sInputsA;
    sOutput.A = sInputsA.A - sqrt(sInputsA.nAvg) ./ sqrt(sInputsB.nAvg) .* sInputsB.A;
