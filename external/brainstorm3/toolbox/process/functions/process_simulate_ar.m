...
function sProcess = GetDescription() %#ok<DEFNU>
    ...
    % === COEFFICIENT MATRIX
    sProcess.options.A.Comment = ['<HTML><BR>Using the ARSIM function from ARFIT Toolbox (T.Schneider):<BR>' ...
                                  ...
                                  '&nbsp;&nbsp;&nbsp;&nbsp; - Ai: [Nsignals x Nsignals]<BR>'];
    sProcess.options.A.Type    = 'textarea';
    sProcess.options.A.Value   = ['A1 = [.2 0 0; .7 .4 0; .1 .8 .4];', 10 ...
                                  'A2 = [.2 0 0; .7 .4 0; .1 .8 .4];', 10 ...
                                  'A = [A1, A2];'];
    % === INTERCEPT
    sProcess.options.b.Comment = '<HTML>Intercept <B>b</B>: &nbsp;&nbsp;[1 x Nsignals]';
    sProcess.options.b.Type    = 'textarea';
    sProcess.options.b.Value   = 'b = [.1 .05 .3];';
    % === NOISE COVARIANCE
    sProcess.options.C.Comment = '<HTML>Noise covariance matrix <B>C</B>: &nbsp;&nbsp;[Nsignals x Nsignals]<BR>';
    sProcess.options.C.Type    = 'textarea';
    sProcess.options.C.Value   = 'C = eye(3,3);';
end
...
function OutputFiles = Run(sProcess, sInputA) %#ok<DEFNU>
    ...
    % === SAVE FILE ===
    % Output filename
    OutputFiles{1} = bst_process('GetNewFilename', bst_fileparts(sStudy.FileName), 'matrix_simar_');
    % Save file
...
% ARSIM function from ARFIT Toolbox
% 
