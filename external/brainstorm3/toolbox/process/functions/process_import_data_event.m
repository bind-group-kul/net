...
function OutputFiles = Run(sProcess, sInputs) %#ok<DEFNU>
    ...
    elseif ~isempty(sInputs)
        ...
        if isRaw
            ...
            FileFormat = 'BST-DATA';
            sFile = in_fopen(FileNames{1}, FileFormat);
        end
    else
        FileNames = {};
    end
    ...
    for iFile = 1:length(FileNames)
        ...
        if ~isempty(sInputs) && (iFile >= 2)
            ...
            if isRaw
                ...
                sFile = in_fopen(FileNames{iFile}, FileFormat, ImportOptions);
            end
    ...
    if ~isempty(OutputFiles)
        ...
        bst_report('Error', sProcess, sInputs, 'No good trials imported from those files.');
    end
end
