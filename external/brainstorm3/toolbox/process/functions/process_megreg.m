...
function sProcess = GetDescription() %#ok<DEFNU>
    ...
    sProcess.options.label2.Comment = '<HTML><BR>Use default channel file :';
...
function [OutputFiles, maxDist] = Run(sProcess, sInputs) %#ok<DEFNU>
    ...
    for iFile = 1:length(ChannelMats)
        ...
        if ~isChanEqual(iFile) || isDebug
            ...
            interpProj = process_ssp('ConvertOldFormat', tmpInterp);
        ...
        for iFile = 1:length(ChannelMats)
            if isfield(ChannelMats{iFile}, 'Projector') && ~isempty(ChannelMats{iFile}.Projector)
                if isempty(Projector)
                    ...
                    bst_report('Warning', sProcess, sInputs, 'Ignoring Projector matrix. Using only the one from the first channel file.');
                end
            end
        end
    end
    ...
    for iInput = 1:length(sInputs)
        ...
        if ~isempty(MegInterp{iFile})
            ...
            if isstruct(DataMat.F)
                DataMat.F.channelmat = ChannelMats{iFile};
