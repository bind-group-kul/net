...
function sProcess = GetDescription() %#ok<DEFNU>
    ...
    sProcess.Comment     = '[Experimental] Classify events';
    sProcess.FileTag     = '';
...
function OutputFiles = Run(sProcess, sInput) %#ok<DEFNU>
    ...
    for iOcc = 1:nOcc
        F(:,iOcc) = in_fread(sFile, sEvent.epochs(iOcc), [sEvent.samples(iOcc), sEvent.samples(iOcc)], iChannels, ImportOptions);
    end
    ...
    for iClass = 1:max(evtClass)
        % Get all the occurrances for this category
