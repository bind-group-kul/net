...
function sProcess = GetDescription() %#ok<DEFNU>
    ...
    sProcess.Comment     = 'Events: Combine stim/response';
    ...
    % Event name
    sProcess.options.combine.Comment = ['<HTML>Example: We have one stim (A) and two responses (B and C).<BR>' ...
    ...
    for iFile = 1:length(sInputs)
        ...
        for iComb = 1:size(combineCell,1)
            ...
            for is = 1:length(tStim)
                ...
                if ~isempty(ir) && (isempty(ior) || (tResp(ir) <= tOtherResp(ior)))
                    iNewStim(end+1) = is;
                    iNewResp(end+1) = ir;
                end
            end
            ...
            if ~isempty(iNewStim)
                ...
                if ~strcmpi(evtNewStimName, 'ignore')
                    ...
                    if isempty(iEvtNewStim)
                        ...
                        iColor = mod(iEvtNewStim - 1, length(ColorTable)) + 1;
                ...
                % === NEW RESP ===
                % Only create new response event if name is not "ignore"
                if ~strcmpi(evtNewRespName, 'ignore')
                    % If new stim event does not exist: create
                    if isempty(iEvtNewResp)
                        ...
                        % Color
                        ColorTable = panel_record('GetEventColorTable');
                        iColor = mod(iEvtNewResp - 1, length(ColorTable)) + 1;
                        sEvent.color = ColorTable(iColor,:);
                        % Add new event to list
