...
function OutputFiles = Run(sProcess, sInputs) %#ok<DEFNU>
    ...
    for i = 1:length(sInputs)
        ...
        % Convert
        [sFile, Messages] = Compute(sFile, recType);
        % Error handling
        ...
        % Add history field
        DataMat = bst_history('add', DataMat, 'ctf', 'Converted to epoched/continuous.');
    ...
    if isInteractive
        ...
        if ~isempty(OutputFiles)
            java_dialog('msgbox', 'Conversion done.');
        end
    end
end
...
%% ===== COMPUTE =====
function [sFile, Messages] = Compute(sFile, recType)
    % ===== PARSE INPUTS =====
    if (nargin < 2)
        recType = 'continuous';
    end
