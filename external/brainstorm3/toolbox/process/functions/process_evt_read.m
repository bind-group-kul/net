...
function sProcess = GetDescription() %#ok<DEFNU>
    ...
    sProcess.Comment     = 'Events: Read from channel';
    sProcess.FileTag     = '';
...
function OutputFiles = Run(sProcess, sInput) %#ok<DEFNU>
    ...
    iChannels = channel_find(sFile.channelmat.Channel, StimChan);
    ...
    % ===== DETECTION =====
    events = Compute(sFile, StimChan, EventsTrackMode, isAcceptZero);
...
%% ===== COMPUTE =====
function events = Compute(sFile, StimChan, EventsTrackMode, isAcceptZero)
    % Parse inputs
    if (nargin < 4)
        isAcceptZero = 0;
    end
    ...
    ch_names = {sFile.channelmat.Channel.Name};
    ...
    if isempty(StimChan) || isequal(StimChan, 'ask') || isequal(StimChan, {'ask'})
        ...
        switch (sFile.format)
            ...
            case '4D'
                % TRIGGER Channels
                iStiChan = channel_find(sFile.channelmat.Channel, 'Stim');
            case {'CTF', 'CTF-CONTINUOUS'}
                % TRIGGER Channels
                iStiChan = channel_find(sFile.channelmat.Channel, 'Stim');
            case 'EEG-BDF'
                % Status Channel
                iStiChan = channel_find(sFile.channelmat.Channel, 'BDF');
        end
    ...
    StimChan = {sFile.channelmat.Channel(iChannels).Name};
    ...
    if isProgressBar
        bst_progress('start', 'Import MEG recordings (FIF raw)', 'Reading events channels...', 0, nbBlocks);
    end
    ...
    for iBlock = 1:nbBlocks
        ...
        % Read block of data
        [tracks, times] = in_fread(sFile, 1, samplesBlock, iChannels);
