...
%    - bst_get('NoiseCovFile', NoiseCovFile)            : Find a HeadModel file in current protocol
%    - bst_get('NoiseCovFile', NoiseCovFile, iStudies)  : Find a HeadModel file in current protocol
...
%    - bst_get('MatlabVersion')         : Matlab version
...
%    - bst_get('DisableOpenGL')         : {0,1} - If 1, do not use OpenGL renderer
...
%% ==== BRAINSTORM CONFIGURATION ====
    ...
    case 'MatlabVersion'
        ...
        argout1 = struct('Version', Version, ...
                         'Release', Release);
...
%% ==== CHANNEL MODALITIES =====
    ...
    case 'ChannelModalities'
        ...
        if ~isempty(sChannel)
            ...
            defList = setdiff(defList, {'EDF','BDF'});
...
%% ==== DATA FOR DATA LIST ====
    ...
    case 'DataForDataList'
        ...
        iFoundData = find(strcmpi(listComments, DataListName));
...
%% ==== ANATOMY DEFAULTS ====
    ...
    case 'AnatomyDefaults'
        ...
        if ~ismember(lower({sTemplates.Name}), 'colin27')
            sTemplates(end+1).FilePath = 'http://neuroimage.usc.edu/brainstorm3_register/getupdate.php?t=Colin27';
            sTemplates(end).Name = 'FSAverage';
        end
        if ~ismember(lower({sTemplates.Name}), 'colin27_2012')
            sTemplates(end+1).FilePath = 'http://neuroimage.usc.edu/brainstorm3_register/getupdate.php?t=Colin27_2012';
            sTemplates(end).Name = 'Colin27_2012';
        end
        if ~ismember(lower({sTemplates.Name}), 'icbm152')
            sTemplates(end+1).FilePath = 'http://neuroimage.usc.edu/brainstorm3_register/getupdate.php?t=ICBM152';
            sTemplates(end).Name = 'ICBM152';
        end
        if ~ismember(lower({sTemplates.Name}), 'fsaverage')
            sTemplates(end+1).FilePath = 'http://neuroimage.usc.edu/brainstorm3_register/getupdate.php?t=FSAverage';
            sTemplates(end).Name = 'FSAverage';
        end
...
%% ==== EEG DEFAULTS ====
    ...
    case 'EegDefaults'
        ...
        eegDefDir = bst_fullfile(bst_get('BrainstormHomeDir'), 'defaults', 'eeg');
        % Get directory
        dirList = dir(bst_fullfile(eegDefDir, '*'));
        ...
        for iDir = 1:length(dirList)
            ...
            % Get full dir
            fulldirName = bst_fullfile(eegDefDir, dirList(iDir).name);
            if isdir(fulldirName)
                % Get files list
                fileList = dir(bst_fullfile(fulldirName, '*channel*.mat'));
                defaultsList = repmat(struct('fullpath','', 'name',''), 0);
                % Find all the valid defaults (channel files)
                for iFile = 1:length(fileList)
                    defaultsList(iFile).fullpath = bst_fullfile(fulldirName, fileList(iFile).name);
...
%% ==== GUI ====
    ...
    case 'DisableOpenGL'
        ...
        elseif exist('isdeployed', 'builtin') && isdeployed && ~strncmp(computer,'MAC',3)
            argout1 = 2;
        else
            argout1 = 0;
        end
...
%% ===== OPENGL BUGS =====
    case 'OpenGLBugs'
        defPref = struct(...
            ...
            'OpenGLWobbleTesselatorBug', -1, ...
            'OpenGLLineSmoothingBug',    -1, ...
            'OpenGLDockingBug',          -1, ...
            'OpenGLClippedImageBug',     -1, ...
            'OpenGLEraseModeBug',        -1);
        bstPref = FillMissingFields(contextName, defPref);
        % Remove the ones that are not available on this computer
        allBugs = fieldnames(bstPref);
        for i = 1:length(allBugs)
            try
                val = opengl(allBugs{i});
            catch
                bstPref.(allBugs{i}) = -2;
            end
        end
        ...
        switch lower(varargin{2})
            ...
            case 'channel'
                argout1 = {...
                    ...
                    {'.pos','.pol','.elp'},        'EEG: Polhemus (*.pos;*.pol;*.elp;*.txt)',   'POLHEMUS'; ...
