...
% ===== NUMBER OF USERS =====
% Read list of users
str = urlread('http://neuroimage.usc.edu/brainstorm3_register/get_userdate.php?c=k9w8cX');
% Extract values
...
% ===== LOG ANALYSIS =====
% Read list of users
str = urlread('http://neuroimage.usc.edu/brainstorm3_register/get_logs.php?c=J7rTwq');
% Extract values
...
iAvg = find((xUpdate >= 2013) & (xUpdate < 2014));
% Plot number of downloads
[hFig(end+1), hAxes] = fig_report(xUpdate(1:end-1), nUpdate(1:end-1), 0, ...
           [2005, max(xUpdate(1:end-1))], [], ...
           sprintf('Downloads per month: Avg(2013)=%d', round(mean(nUpdate(iAvg)))), [], 'Downloads per month', ...
...
% Read list of users
str = urlread('http://neuroimage.usc.edu/brainstorm3_register/get_posts.php?c=3Emzpjt0');
% Extract values
dates = textscan(str, '%d %d');
dates = double([dates{1}, dates{2}]);
dates = dates(:,1) + (dates(:,2)-1)./12;
...
nPubli = [   2    2    1    1    3    5    5   11   10   18   19   33   37   52];
% Plot figure
hFig(end+1) = fig_report(year, nPubli, 1, ...
           [2000 max(year)], [], ...
           sprintf('Peer-reviewed articles and book chapters: %d', sum(nPubli)), [], 'Publications per year', ...
