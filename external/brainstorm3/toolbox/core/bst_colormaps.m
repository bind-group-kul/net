...
%               bst_colormaps('ConfigureColorbar',    hFig,     ColormapType, DataType)
...
function CreateColormapMenu(jMenu, ColormapType)
    ...
    % Colormaps list
    cmapList = {'cmap_rbw', 'hot', 'cmap_hot2', 'copper', 'bone', 'gray', 'pink', 'hsv', 'jet', 'cmap_jetinv', 'cmap_ns_green', 'cmap_ns_white', 'cmap_ns_grey'};
    iconList = [IconLoader.ICON_COLORMAP_RBW, IconLoader.ICON_COLORMAP_HOT, IconLoader.ICON_COLORMAP_HOT, IconLoader.ICON_COLORMAP_COPPER, IconLoader.ICON_COLORMAP_BONE, ...
                IconLoader.ICON_COLORMAP_GREY, IconLoader.ICON_COLORMAP_PINK, IconLoader.ICON_COLORMAP_HSV, IconLoader.ICON_COLORMAP_JET, ...
                IconLoader.ICON_COLORMAP_JET, IconLoader.ICON_COLORMAP_NEUROSPEED, IconLoader.ICON_COLORMAP_NEUROSPEED, IconLoader.ICON_COLORMAP_NEUROSPEED];
    isCustom = 1;
    for i = 1:length(cmapList)
        % If the colormap #i is currently used for this surface : check the menu
        if strcmpi(cmapList{i}, sColormap.Name)
            isSelected = 1;
            isCustom = 0;
        else
            isSelected = 0;
    ...
    % Add "Custom..." menu (and select it if colormap is user defined)
    jItem = gui_component('MenuItem', jMenuColormap, [], 'Custom...', IconLoader.ICON_COLORMAP_CUSTOM, [], @(h,ev)SetColormapName(ColormapType, ''), []);
    jItem.setSelected(isCustom);
...
function CreateAllMenus(jMenu, hFig) %#ok<DEFNU>
    import org.brainstorm.icon.*;
    % Parse inputs
    if (nargin < 2)
        hFig = [];
    ...
    if isempty(hFig) || ismember('anatomy', AllTypes)
        gui_component('Menu', jMenu, [], [spre 'Anatomy'], IconLoader.ICON_COLORMAP_ANATOMY, '', @(h,ev)CreateColormapMenu(ev.getSource(), 'anatomy'), []);
    end
    if isempty(hFig) || ismember('eeg', AllTypes)
        gui_component('Menu', jMenu, [], [spre 'EEG Recordings'], IconLoader.ICON_COLORMAP_RECORDINGS, '', @(h,ev)CreateColormapMenu(ev.getSource(), 'eeg'), []);
    end
    if isempty(hFig) || ismember('meg', AllTypes)
        gui_component('Menu', jMenu, [], [spre 'MEG Recordings'], IconLoader.ICON_COLORMAP_RECORDINGS, '', @(h,ev)CreateColormapMenu(ev.getSource(), 'meg'), []);
    end
    if isempty(hFig) || ismember('source', AllTypes)
        gui_component('Menu', jMenu, [], [spre 'Sources'], IconLoader.ICON_COLORMAP_SOURCES, '', @(h,ev)CreateColormapMenu(ev.getSource(), 'source'), []);
    end
    if isempty(hFig) || ismember('stat1', AllTypes)
        gui_component('Menu', jMenu, [], [spre 'Stat 1'], IconLoader.ICON_COLORMAP_STAT, '', @(h,ev)CreateColormapMenu(ev.getSource(), 'stat1'), []);
    end
    if isempty(hFig) || ismember('stat2', AllTypes)
        gui_component('Menu', jMenu, [], [spre 'Stat 2'], IconLoader.ICON_COLORMAP_STAT, '', @(h,ev)CreateColormapMenu(ev.getSource(), 'stat2'), []);
    end
    if isempty(hFig) || ismember('time', AllTypes)
        gui_component('Menu', jMenu, [], [spre 'Time'], IconLoader.ICON_COLORMAP_TIME, '', @(h,ev)CreateColormapMenu(ev.getSource(), 'time'), []);
    end
    if isempty(hFig) || ismember('timefreq', AllTypes)
        gui_component('Menu', jMenu, [], [spre 'Timefreq'], IconLoader.ICON_COLORMAP_TIMEFREQ, '', @(h,ev)CreateColormapMenu(ev.getSource(), 'timefreq'), []);
    end
    if isempty(hFig) || ismember('connect1', AllTypes)
        gui_component('Menu', jMenu, [], [spre 'Connect 1xN'], IconLoader.ICON_COLORMAP_CONNECT, '', @(h,ev)CreateColormapMenu(ev.getSource(), 'connect1'), []);
    end
    if isempty(hFig) || ismember('connectn', AllTypes)
        gui_component('Menu', jMenu, [], [spre 'Connect NxN'], IconLoader.ICON_COLORMAP_CONNECT, '', @(h,ev)CreateColormapMenu(ev.getSource(), 'connectn'), []);
    end
    if isempty(hFig) || ismember('pac', AllTypes)
        gui_component('Menu', jMenu, [], [spre 'PAC'], IconLoader.ICON_COLORMAP_PAC, '', @(h,ev)CreateColormapMenu(ev.getSource(), 'pac'), []);
    end
end
...
function SetColormapName(ColormapType, colormapName)
    ...
    if isempty(colormapName)
        ...
        newColormap = get(hTmp, 'Colormap');
        % Else : update colormap
        sColormap.CMap = newColormap;
        sColormap.Name = '';
    % Else : named colormap
    else
...
function ConfigureColorbar(hFig, ColormapType, DataType) %#ok<DEFNU>
    global GlobalData;
    ...
    if ~isempty(hColorbar)
        ...
        if ~isempty(iTicks)
            ...
            YTick = unique([flipdim(0:-tickSpace/fFactor:dataBounds(1), 2), 0, 0:tickSpace/fFactor:dataBounds(2)]);
...
function SetColorbarVisible(hFig, isVisible) %#ok<DEFNU>
    ...
    if isVisible && isempty(hColorbar)
        ...
        ResizeCallback = get(hFig, 'ResizeFcn');
        ResizeCallback(hFig, []);
...
function AddColormapToFigure(hFig, ColormapType) %#ok<DEFNU>
    % Get existing list
    ColormapInfo = getappdata(hFig, 'Colormap');
