...
%% ===== UPDATE GLOBAL STRUCTURES =====
        ...
        if ~isempty(iNewStudy)
            ...
            if ~isempty(sNewChannel) && ~file_compare(sNewChannel.FileName, oldChannelFile)
                ...
                ChannelFileFull = bst_fullfile(ProtocolInfo.STUDIES, newChannelFile);
                ChannelMat = load(ChannelFileFull, 'Channel');
        ...
        % Update time window
        isTimeCoherent = bst_memory('CheckTimeWindows');
            ...
            switch(Figure.Id.Type)
                ...
                case 'DataTimeSeries'
                    ...
                    elseif ~isempty(Figure.Id.Modality) && (Figure.Id.Modality(1) ~= '$')
                        ...
                        % Get previous display factor
                        if isfield(Figure.Handles, 'DisplayFactor')
                            oldDispFactor = Figure.Handles.DisplayFactor;
                        end
                        % Get figure info structure
                        TsInfo = getappdata(Figure.hFigure, 'TsInfo');
                        % Call plot function again for new time series
                        view_timeseries(GlobalData.DataSet(iDS).DataFile, Figure.Id.Modality, TsInfo.RowNames, Figure.hFigure);
                        % Update "isStatic" status
                        setappdata(Figure.hFigure, 'isStatic', (GlobalData.DataSet(iDS).Measures.NumberOfSamples <= 2));
                        % Get new display factor
                        if isfield(GlobalData.DataSet(iDS).Figure(iFig).Handles, 'DisplayFactor')
                            newDispFactor = GlobalData.DataSet(iDS).Figure(iFig).Handles.DisplayFactor;
                        end
%                         % Update display factor (restore the previous one)
%                         if ~isempty(oldDispFactor) && ~isempty(newDispFactor)
%                             figure_timeseries('UpdateTimeSeriesFactor', Figure.hFigure, oldDispFactor / newDispFactor, 0);
%                         end
                    else
                        isFigureRemoved = 1;
                    end
