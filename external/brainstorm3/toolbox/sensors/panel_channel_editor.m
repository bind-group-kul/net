...
function bstPanelNew = CreatePanel(ChannelFile, DataFile) %#ok<DEFNU>
    ...
    tableListTypes = {'(Delete)', 'MEG', 'MEG MAG', 'MEG GRAD', 'MEG REF', 'EEG', 'EEG REF', 'ECOG', 'SEEG', 'ECG', 'EOG', 'EMG', 'Stim', 'Misc'};
    jComboTypes = gui_component('ComboBox', [], [], [], {tableListTypes}, [], [], []);
    jComboTypes.setEditable(true);
    jTableChannel.getColumnModel.getColumn(2 + isChannelFlag).setCellEditor(DefaultCellEditor(jComboTypes));
    % COLUMN 4 : COMMENT   
    if ~isChannelFlag
        jTableChannel.getColumnModel.getColumn(3 + isChannelFlag).setPreferredWidth(70);
    else
        jTableChannel.getColumnModel.getColumn(3 + isChannelFlag).setPreferredWidth(150);
    end
    % COLUMN 5-NbColumns : LOC, ORIENTATION
    if ~isChannelFlag
        for iCol = (4+isChannelFlag):nbColumns-1
...
function SaveChannelFile()
    ...
    % Update associated raw links
    UpdateRawLinks(GlobalData.ChannelEditor.ChannelFile, ChannelMat);
end


%% ===== UPDATE RAW LINKS =====
function UpdateRawLinks(ChannelFile, ChannelMat)
    % Get dependent data files for this file
    DataFiles = bst_get('DataForChannelFile', ChannelFile);
    % If there are available files
    if ~isempty(DataFiles)
        % Find raw files
        iRaw = find(~cellfun(@(c)isempty(strfind(c,'_0raw')), DataFiles));
        % Look for raw files
        for i = 1:length(iRaw)
            % Load raw file
            RawFile = file_fullpath(DataFiles{iRaw(i)});
            DataMat = in_bst_data(RawFile, 'F');
            % Update sFile
            F = DataMat.F;
            F.channelmat = ChannelMat;
            % Save modifications
            save(RawFile, 'F', '-append');
        end
    end
end
...
function Channel = GetTableData(jTableChannel)
    ...
    for iChannel = 1:nbChannels
        ...
        Channel(iChannel).Type = char(jTableChannel.getValueAt(iChannel - 1, 2 + isChannelFlag));
        % Do not update comment if it initially contains data
        if isempty(Channel(iChannel).Comment) || ischar(Channel(iChannel).Comment)
            Channel(iChannel).Comment = char(jTableChannel.getValueAt(iChannel - 1, 3 + isChannelFlag));
        end
        % Numeric values
        if ~isChannelFlag
            % Location
            ...
            for iOrient = GlobalData.ChannelEditor.OrientColumns
                ...
                if ~isempty(jOrient)
                    Channel(iChannel).Orient = [Channel(iChannel).Orient, [jOrient(1);jOrient(2);jOrient(3)] ];
                end
            end
            % Weight
            Channel(iChannel).Weight = [];
            jOrient = jTableChannel.getValueAt(iChannel - 1, nbColumns - 1);
            for i = 1:length(jOrient)
                Channel(iChannel).Weight(i) = double(jOrient(i));
            end
        end
    end
    bst_progress('stop');
...
function SetChannelsField(target)
    ...
    isChannelFlag = ~isempty(GlobalData.ChannelEditor.DataFile);
    for i = 1:length(iSelChan)
        ctrl.jTableChannel.setValueAt(newType, iSelChan(i) - 1, iColumn + isChannelFlag);
    end
    % Repaint table
    ctrl.jTableChannel.repaint();
end
...
function UpdateChannelFlag(DataFile, ChannelFlag, varargin)
    ...
    for iFig = 1:length(GlobalData.DataSet(iDS).Figure)               
        ...
        % The way to reload depends on the figure type
        switch Figure.Id.Type
            case 'DataTimeSeries' 
                % Update the display
                view_timeseries(DataFile, Figure.Id.Modality);
            case 'ResultsTimeSeries'
                % Nothing to do
            case '3DViz'
%                 % Update selected channels
%                 figure_3d('UpdateEditor', iDS, iFig);
%                 % Get Surfaces displayed in this figure
%                 hFig = Figure.hFigure;
%                 TessInfo = getappdata(hFig, 'Surface');
%                 if ~isempty(TessInfo)
%                     sourcesList = [TessInfo.DataSource];
%                     iData = find(strcmpi({sourcesList.Type}, 'Data'));
%                     % If at least one surface has some recordings displayed on it
%                     if ~isempty(iData)
%                         iData = iData(1);
%                         src = sourcesList(iData);
%                         isStat = strcmpi(file_gettype(src.FileName), 'pdata');
%                         % Update data source
%                         panel_surface('SetSurfaceData', hFig, iRecordings, src.Type, src.FileName, isStat);
%                     end
%                 end
                warning('TODO');
                
            case 'Topography'
                % Get the time-frequency description structure
                TfInfo = getappdata(Figure.hFigure, 'Timefreq');
                % Call view_topography to update the display
                if ~isempty(TfInfo) && ~isempty(TfInfo.FileName)
                    view_topography(TfInfo.FileName, Figure.Id.Modality, Figure.Id.SubType);
                else
                    view_topography(GlobalData.DataSet(iDS).DataFile, Figure.Id.Modality, Figure.Id.SubType);
                end
            case 'MriViewer'
                % Nothing to do       
            case {'Timefreq', 'Spectrum', 'Connect', 'Pac'}
                % Nothing to do
        end
