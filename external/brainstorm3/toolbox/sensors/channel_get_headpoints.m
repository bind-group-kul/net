...
%     % Ask user if we should use the EEG points as head points
%     % Display confirm message
%     isOk = java_dialog('confirm', ['Warning: Not enough head points available.' 10 10 ...
%                sprintf('Use %d EEG electrodes as head points ?', length(iEeg)), 10 10], ...
%                'Get head points');
%     % If confirmed: add EEG as head points
%     if isOk
%         % Add EEG points
%         HeadPoints = channel_add_headpoints(file_fullpath(ChannelFile), 'IncludeEeg');
%     end
