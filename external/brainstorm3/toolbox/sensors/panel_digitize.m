...
function [bstPanelNew, panelName] = CreatePanel() %#ok<DEFNU>
    ...
    ctrl = struct('jMenuEeg',              jMenuEeg, ...
                  ...
                  'jLabelMessage',         jLabelCoilMessage, ...
...
function isOk = EditSettings()
    ...
    res = java_dialog('input', ...
            ...
            {DigitizeOptions.ComPort, ...
             num2str(DigitizeOptions.ComRate), ...
             num2str(DigitizeOptions.ComByteCount), ...
    ...
    % Check values
    if (length(res) < 6) || isempty(res{1}) || isnan(str2double(res{2})) || isnan(str2double(res{3})) || ~ismember(str2double(res{4}), [0 1]) || isnan(str2double(res{5})) || ~ismember(str2double(res{6}), [0 1])
        bst_error('Invalid values.', 'Digitize', 0);
        return;
    end
    % Get entered values
    DigitizeOptions.ComPort      = res{1};
    DigitizeOptions.ComRate      = str2double(res{2});
    DigitizeOptions.ComByteCount = str2double(res{3});
    DigitizeOptions.isMEG        = str2double(res{4});
    DigitizeOptions.nFidSets     = str2double(res{5});
    DigitizeOptions.isBeep       = str2double(res{6});
...
function DeletePoint_Callback(h, ev) %#ok<INUSD>
    ...
    if Digitize.Mode == 7
        ...
        if size(Digitize.Points.EEG,1) == nEEG
            % all EEG points have been collected, delete the last one
            iPoint = nEEG;
        end                
...
function ComputeTransform()
    ...
    if DigitizeOptions.isMEG
        ...
        if (size(Digitize.Points.hpiN, 1) > 1)
            ...
            if any(abs(normPoint) > .002)
                ctrl.jLabelFidMessage.setText('Difference error exceeds 2 mm');
            end
        end
        ...
        if (size(Digitize.Points.nasion, 1) > 1)
            ...
            if any(abs(normPoint) > .005)
                ctrl.jLabelMessage.setText('Difference error exceeds 5 mm');
            end
        end
        % find the average across collections to compute the transformation
    ...
    sMRI.SCS = cs_mri2scs(sMRI);
    T = sMRI.SCS.T ./ 1000; % mm => m
    R = sMRI.SCS.R;
    Origin = sMRI.SCS.Origin ./ 1000; %#ok<NASGU> % mm => m
...
function BytesAvailable_Callback(h, ev) %#ok<INUSD>
    ...
    if DigitizeOptions.isBeep && (Digitize.Mode ~= 8)
