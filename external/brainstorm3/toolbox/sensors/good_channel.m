...
for iType = 1:length(channelTypes)
    ...
    switch lower(channelType)
        % MEG: all sensors
        case {'meg', 'vectorview306', 'ctf', '4d', 'kit'}
        ...
        case {'meg grad2', 'meg grad3'}
            ...
            elseif ~isempty(iChan3) && isempty(iChan2)
                ...
                % Loop through all those names, and keep only the ones that have the two gradiometers available
            ...
            if strcmpi(channelType, 'meg grad2')
                iChannelType = iGrad2;
            else
                iChannelType = iGrad3;
            end
