function [ChannelMat, R, T] = channel_align_auto(ChannelFile, ChannelMat, isWarning, isConfirm)
% CHANNEL_ALIGN_AUTO: Aligns the channels to the scalp using Polhemus points.
%
% USAGE:  [ChannelMat, R, T] = channel_align_auto(ChannelFile, ChannelMat=[], isWarning=1, isConfirm=1)
%
% DESCRIPTION: 
...
if isSave && (isWarning || isConfirm)
    ...
    if ~isempty(iMeg)
        ...
        AllMod = intersect(AllMod, {'EEG','SEEG','ECOG'});
        if ~isempty(AllMod)
            modality = AllMod{1};
        else
            modality = [];
