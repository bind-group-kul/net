...
function GUI = CreateWindow() %#ok<DEFNU>
    ...
    jMenuColormaps = gui_component('Menu', jMenuBar, [], 'Colormaps', [], [], [], menuSize);
        bst_colormaps('CreateAllMenus', jMenuColormaps);
    ...
    jMenuSupport = gui_component('Menu', jMenuBar, [], 'Help', [], [], [], menuSize);
        ...
        jMenuOpenmeeg = gui_component('Menu', jMenuSupport, [], 'Update OpenMEEG', [], [], [], []);
            ...
            if strcmpi(bst_get('OsType',0), 'win64')
                ...
                gui_component('MenuItem', jMenuOpenmeeg, [], 'Download Visual C++', [], [], @(h,ev)web('http://www.microsoft.com/en-us/download/details.aspx?id=14632'), []);
            end
            jMenuOpenmeeg.addSeparator();
            gui_component('MenuItem', jMenuOpenmeeg, [], 'OpenMEEG help', [], [], @(h,ev)web('http://neuroimage.usc.edu/brainstorm/Tutorials/TutBem'), []);
        ...
        % WEBSITE
        gui_component('MenuItem', jMenuSupport, [], 'Brainstorm website', [], [], @(h,ev)web('http://neuroimage.usc.edu/brainstorm/'), []);
        gui_component('MenuItem', jMenuSupport, [], 'Brainstorm forum', [], [], @(h,ev)web('http://neuroimage.usc.edu/forums/'), []);
...
%% ===== SET DATABASE FOLDER =====
function BrainstormDbDir = SetDatabaseFolder(varargin)
...
function DownloadOpenmeeg()
    ...
    % Open web browser
    try
        web('http://openmeeg.gforge.inria.fr/download/');
    catch
    end
end
