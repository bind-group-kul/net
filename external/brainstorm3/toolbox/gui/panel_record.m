...
function bstPanelNew = CreatePanel() %#ok<DEFNU>
    ...
    jPanelTime.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createEmptyBorder(7,7,0,7), jBorder));
        ...
        java_setcb(jSpinnerEpoch, 'StateChangedCallback',  @(h,ev)UpdateTime());
        % Time start
    ...
    jPanelEvent.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createEmptyBorder(0,7,7,7), jBorder));
        ...
        gui_component('MenuItem', jMenu, [], 'Convert to simple event', IconLoader.ICON_EVT_OCCUR, [], @(h,ev)bst_call(@EventConvertToSimple), []);
        ...
        % SSP
        jMenu = gui_component('Menu', jMenuBar, [], 'SSP', IconLoader.ICON_MENU, [], [], 11);
        ...
        jMenu.addSeparator();
        jItemSspEog  = gui_component('MenuItem', jMenu, [], 'Compute SSP: Eye blinks', IconLoader.ICON_EMPTY, [], @(h,ev)CallProcessOnRaw('process_ssp_eog'), []);
        jItemSspEcg  = gui_component('MenuItem', jMenu, [], 'Compute SSP: Heartbeats', IconLoader.ICON_EMPTY, [], @(h,ev)CallProcessOnRaw('process_ssp_ecg'), []);
        jItemSsp     = gui_component('MenuItem', jMenu, [], 'Compute SSP: Generic',    IconLoader.ICON_EMPTY, [], @(h,ev)CallProcessOnRaw('process_ssp'), []);
        jMenu.addSeparator();
...
function UpdateDisplayOptions(hFig)
    ...
    isTopo = strcmpi(TsInfo.DisplayMode, 'topography');
    ctrl.jButtonDispMode.setEnabled(~isTopo);
    ctrl.jButtonUniform.setEnabled(~isTopo);
    ctrl.jMenuMontage.setEnabled(1);
    % Update montage name
    if strcmpi(TsInfo.Modality, 'Sources') || ~isempty(TsInfo.RowNames)
        ctrl.jMenuMontage.setVisible(0);
    else
        ctrl.jMenuMontage.setVisible(1);
        if isempty(TsInfo.MontageName)
            ctrl.jMenuMontage.setText('All');
        elseif strcmpi(TsInfo.MontageName, 'Average reference')
            ctrl.jMenuMontage.setText('Avg Ref');
        else
            ctrl.jMenuMontage.setText(TsInfo.MontageName);
        end
    end
    % Update display mode
...
function UpdatePanel(hFig)
    ...
    % Get type of 
    if ~isempty(iFig)
        isData = strcmpi(GlobalData.DataSet(iDS).Figure(iFig).Id.Type, 'DataTimeSeries');
    else
        isData = 0;
    end
    ...
    if isRaw
        ...
        sFile = GlobalData.DataSet(iDS).Measures.sFile;
        % CTF Compensations
        isCtfCompCheck  = isfield(sFile.channelmat, 'MegRefCoef') && ~isempty(sFile.channelmat.MegRefCoef) && ~isempty(sFile.prop.currCtfComp) && (sFile.prop.currCtfComp == 0);
    ...
    % Enable/disable SSP menus
    gui_enable([ctrl.jItemSspEog, ctrl.jItemSspEcg, ctrl.jItemSsp, ctrl.jItemSspSel], isRaw);
...
%% ===== READ RAW BLOCK =====
function [F, TimeVector, smpBlock] = ReadRawBlock(sFile, iEpoch, TimeRange, DisplayMessages, UseCtfComp, RemoveBaseline) %#ok<DEFNU>
    ...
    [F, TimeVector] = in_fread(sFile, iEpoch, smpBlock, [], ImportOptions);
end
...
function UpdateEventsList()
    ...
    for iEvent = 1:length(events)
        ...
        if isfield(events(iEvent), 'color') && ~isempty(events(iEvent).color)
...
function iEvent = EventTypeAdd(sEvent)
    ...
    if (nargin == 1) && isstruct(sEvent)
        ...
        if ~isempty(strfind(lower(newLabel), 'bad'))
            newColor = [1 0 0];
        % Else: Get default new color
        else
            ColorTable = GetEventColorTable();
            iColor = mod(iEvent - 1, length(ColorTable)) + 1;
...
function EventTypesDuplicate()
    ...
    for i = 1:length(iEvents)
        ...
        % Change color
        iColor = mod(iCopy(i) - 1, length(ColorTable)) + 1;
        events(iCopy(i)).color = ColorTable(iColor,:);
    end
    % Update dataset
    SetEvents(events);
    % Update events list
...
function EventConvertToSimple()
    ...
    res = java_dialog('question', ...
        'Keep only the first of last samples of the extended events ?', ...
        'Convert event type', [], {'First', 'Last', 'Cancel'}, 'First');
...
%         % Ignore beginning and end of the time window
%         if any(abs(newTime - GlobalData.UserTimeWindow.Time) < 1e-8)
%             return
%         end
...
%         if ~isempty(sEvent.samples) && (any((sEvent.times(1,:) >= newTime(1)) & (sEvent.times(1,:) <= newTime(2)) & isEpochOk) || ...
%                                         any((sEvent.times(2,:) >= newTime(1)) & (sEvent.times(2,:) <= newTime(2)) & isEpochOk) || ...
...
function ImportEvents(varargin)
    ...
    % Import new file
    if isRaw
        [GlobalData.DataSet(iDS).Measures.sFile, newEvents] = import_events(GlobalData.DataSet(iDS).Measures.sFile);
    else
        ...
        % Import new file
        [sFile, newEvents] = import_events(sFile);
...
function [badSeg, badEpochs] = GetBadSegments(sFile) %#ok<DEFNU>
    ...
    for iEvt = 1:length(events)
        ...
        if ~isempty(strfind(lower(events(iEvt).label), 'bad')) && ~isempty(events(iEvt).samples)
            % If extended event
...
%     % If no modifications: exit
%     if isequal(GlobalData.DataSet(iDS).Measures.sFile, DataMat.F)
%         bst_progress('stop');
%         return;
%     end
    ...
    % Only for SSP processes
    if ismember(ProcessName, {'process_ssp', 'process_ssp_eog', 'process_ssp_ecg'})
        % Update SSP Selection interface
        if ~isempty(EditSspPanel)
            EditSspPanel.InitProjector = GlobalData.DataSet(iDS).Measures.sFile.channelmat.Projector;
...
%% ===== DELETE RAW FILE =====
function DeleteRawFile(DataFile) %#ok<DEFNU>
