...
function bstPanelNew = CreatePanel(action) %#ok<DEFNU>
    ...
    if strcmpi(action, 'create') || strcmpi(action, 'edit')
        ...
        ctrl.jRadioChannelIndividual     = JRadioButton('No, use one channel file per condition/run (MEG).');
        ctrl.jRadioChannelDefaultSubject = JRadioButton('Yes, use one channel file per subject (MEG,EEG).');
...
%  =================================================================================
    ...
    function GlobalWarning(varargin)
        ...
        res = java_dialog('question', ['Warning: We do not recommend this option.' 10 10 ...
                                       'Sharing the same electrodes cap for all the subjects is fast but can be unaccurate.' 10 ...
