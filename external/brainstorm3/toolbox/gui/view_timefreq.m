function [hFig, iDS, iFig] = view_timefreq(TimefreqFile, DisplayMode, RowName, isNewFigure)
% VIEW_TIMEFREQ: Display times frequency maps in a new figure.
%
% USAGE: [hFig, iDS, iFig] = view_timefreq(TimefreqFile, DisplayMode='SingleSensor', RowName=[], isNewFigure=0)
%
% INPUT: 
%     - TimefreqFile : Path to time-frequency file to visualize
%     - DisplayMode  : {'SingleSensor', '2DLayout', '2DLayoutOpt'}
...
% Check display type
if any(strcmpi(DisplayMode, {'2DLayout', '2DLayoutOpt'})) && (isempty(Modality) || ~ismember(Modality, {'MEG','MEG GRAD','MEG MAG','EEG'}))
...
if ~isempty(RowName) && ischar(RowName) && ~ismember(RowName, GlobalData.DataSet(iDS).Timefreq(iTimefreq).RowNames)
...
elseif iscell(GlobalData.DataSet(iDS).Timefreq(iTimefreq).RowNames)
