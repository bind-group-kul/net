...
function UpdatePanel()
    ...
    if ~isempty(CoordinatesSelector) && ~isempty(CoordinatesSelector.MRI)
        ctrl.jLabelCoordMriX.setText(sprintf('%3.2f', CoordinatesSelector.MRI(1)));
        ctrl.jLabelCoordMriY.setText(sprintf('%3.2f', CoordinatesSelector.MRI(2)));
        ctrl.jLabelCoordMriZ.setText(sprintf('%3.2f', CoordinatesSelector.MRI(3)));
    else
    ...
    if ~isempty(CoordinatesSelector) && ~isempty(CoordinatesSelector.SCS)
        ctrl.jLabelCoordScsX.setText(sprintf('%3.2f', CoordinatesSelector.SCS(1)));
        ctrl.jLabelCoordScsY.setText(sprintf('%3.2f', CoordinatesSelector.SCS(2)));
        ctrl.jLabelCoordScsZ.setText(sprintf('%3.2f', CoordinatesSelector.SCS(3)));
    else
    ...
    if ~isempty(CoordinatesSelector) && ~isempty(CoordinatesSelector.MNI)
        ctrl.jLabelCoordMniX.setText(sprintf('%3.2f', CoordinatesSelector.MNI(1)));
        ctrl.jLabelCoordMniY.setText(sprintf('%3.2f', CoordinatesSelector.MNI(2)));
        ctrl.jLabelCoordMniZ.setText(sprintf('%3.2f', CoordinatesSelector.MNI(3)));
    else
...
function vi = SelectPoint(hFig, AcceptMri) %#ok<DEFNU>
    ...
    % We have the coordinates in SCS, we need them in MRI CS
    % === MRI CS ===
    if ~isempty(sMri) && isfield(sMri, 'SCS') && ~isempty(sMri.SCS)
        mriLoc = cs_scs2mri(sMri, scsLoc * 1000);
    else
        mriLoc = [];
    end
    % === MNI CS ===
    if ~isempty(sMri) && isfield(sMri, 'NCS') && isfield(sMri.NCS, 'R') && ~isempty(sMri.NCS.R)
        mniLoc = cs_mri2mni(sMri, mriLoc);
    else
        mniLoc = [];
    end
    % Save selected point
    CoordinatesSelector.MRI = mriLoc;
    CoordinatesSelector.SCS = scsLoc * 1000;
    CoordinatesSelector.MNI = mniLoc;
...
function ViewInMriViewer(varargin)
    ...
    figure_mri('SetLocation', 'mm', hFig, [], CoordinatesSelector.MRI');
    % Close progress bar
    bst_progress('stop');
end
