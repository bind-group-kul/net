...
function bstPanelNew = CreatePanel() %#ok<DEFNU>
    ...
    jMenuBar = gui_component('MenuBar', jPanelTop, BorderLayout.NORTH);
        ...
        % Add/Remove button
        gui_component('ToolbarButton', jToolbar, [], [], {IconLoader.ICON_SURFACE_ADD, TB_DIM},    'Add a surface',              @ButtonAddSurfaceCallback);
        gui_component('ToolbarButton', jToolbar, [], [], {IconLoader.ICON_SURFACE_REMOVE, TB_DIM}, 'Remove surface from figure', @ButtonRemoveSurfaceCallback);
...
function isOk = SetSurfaceData(hFig, iTess, dataType, dataFile, isStat) %#ok<DEFNU>
    ...
    switch (dataType)
        ...
        case 'Timefreq'
            ...
            % Data type
            [TfInfo.Function, ColormapType] = process_tf_measure('GetDefaultFunction', GlobalData.DataSet(iDS).Timefreq(iTimefreq).Method);
            % Set figure data
            setappdata(hFig, 'Timefreq', TfInfo);
    ...
    for i = 1:length(iSurfaces)
        ...
        switch (TessInfo(iTess).DataSource.Type)
            ...
            case 'Source'
                ...
                elseif (size(TessInfo(iTess).Data, 1) ~= TessInfo(iTess).nVertices)
            ...
            case 'Timefreq'
                ...
                elseif (size(TessInfo(iTess).Data, 1) ~= TessInfo(iTess).nVertices) && ~strcmpi(TessInfo(iTess).Name, 'Anatomy')
                    bst_error(sprintf(['Number of sources (%d) is different from number of vertices (%d).\n\n' ...
                              'Please compute the sources again.'], size(TessInfo(iTess).Data, 1), TessInfo(iTess).nVertices), 'Data mismatch', 0);
                    isOk = 0;
                    return;
...
function [Vertices, Faces, VertexNormals, iVisibleVert] = GetSurfaceVertices(hPatch) %#ok<DEFNU>
    Faces    = get(hPatch, 'Faces');
    Vertices = get(hPatch, 'Vertices');
    VertexNormals = get(hPatch, 'VertexNormals');
    ...
    switch (TessInfo(iTess).DataSource.Type)
       case 'Data'
        ...
        case 'Source'
            ...
            isVolumeGrid = ismember(GlobalData.DataSet(iDS).Results(iResult).HeadModelType, {'volume', 'dba'});
        case 'Timefreq'
            % Get cortex surface
            sSubject = bst_get('MriFile', sMri.FileName);
            SurfaceFile = sSubject.Surface(sSubject.iCortex).FileName;
