...
function ResizeCallback(hFig, ev)
    ...
    if isempty(findobj(hAxes, '-depth', 1, 'tag', 'TimefreqSurf'))
        return
    end
...
%  ===========================================================================
%% ===== GET FIGURE DATA =====
function [Time, Freqs, TfInfo, TF, RowNames, FullTimeVector, DataType, LowFreq] = GetFigureData(hFig, TimeDef)
    global GlobalData;
    % Parse inputs
    ...
    if (nargout >= 4)
        ...
        % Get data
        [TF, iTimeBands, iRow] = bst_memory('GetTimefreqValues', iDS, iTimefreq, FigRowName, TfInfo.iFreqs, iTime, TfInfo.Function);
        % Get specific RowNames
        if ~isempty(FigRowName)
            RowNames = FigRowName;
...
function UpdateFigurePlot(hFig, isForced)
    ...
    if TfInfo.HideEdgeEffects
        ...
        if ~isempty(tfOptions) && isfield(tfOptions, 'MorletFc') && isfield(tfOptions, 'MorletFwhmTc')
            ...
            TF_mask = double(TF_mask & flipdim(TF_mask, 2));
            clear TF_timeres;
        end
    end
    ...
    bst_colormaps('ConfigureColorbar', hFig, ColormapInfo.Type, 'timefreq');
end
        ...
        if ~isempty(iFreq)
            for i = 1:length(iFreq)
                if (iFreq(i) == round(iFreq(i))) && (iFreq(i) > 0) && (iFreq(i) <= length(Freqs))
                    strFreqs{i} = num2str(round(Freqs(iFreq(i))));
                else
                    strFreqs{i} = '';
                end
            end
...
function PlotAllSensors(hFig, RowNames, TF, TF_mask, MinMaxVal, is2DLayout)
    ...
    for iRow = 1:length(RowNames)
        ...
        % Add text legend
        text(X(iRow), Y(iRow)-.51*axesSize(2), RowNames{iRow}, ...
             'FontSize',            fontSize, ...
             'FontUnits',           'points', ...
             'Interpreter',         'none', ...
...
function ImageClicked_Callback(hFig, FileName, RowName)
    ...
    % CLICK: Display TF decomposition of the sensor
        ...
        view_timefreq(FileName, 'SingleSensor', RowName);
    end
end
...
function [X,Y,axesSize] = GetLayoutPositions(hFig, RowNames, is2DLayout)
    ...
    if is2DLayout
        ...
        selChan = channel_find(GlobalData.DataSet(iDS).Channel, RowNames);
