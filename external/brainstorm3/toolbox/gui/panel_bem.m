...
function [bstPanelNew, panelName] = CreatePanel()  %#ok<DEFNU>  
    ...
    jPanelVert = gui_river([4,4], [3,15,10,10], 'Number of vertices per layer');
        ...
        jTextVertScalp = gui_component('texttime', jPanelVert, 'tab', num2str(BemOptions.nvert(1), '%d'), [], [], [], []);
        % Outer skull
        gui_component('label', jPanelVert, 'br', 'Outer skull: ', [], [], [], []);
        jTextVertOuter = gui_component('texttime', jPanelVert, 'tab', num2str(BemOptions.nvert(2), '%d'), [], [], [], []);
        % Inner skull
        gui_component('label', jPanelVert, 'br', 'Inner skull: ', [], [], [], []);
        jTextVertInner = gui_component('texttime', jPanelVert, 'tab', num2str(BemOptions.nvert(3), '%d'), [], [], [], []);        
...
%         jTextThickInner = gui_component('texttime', jPanelThick, 'tab', num2str(BemOptions.thickness(3), '%d'), [], [], [], []);
    ...
    ctrl = struct('BemOptions',       BemOptions, ...
                  'jTextVertScalp',   jTextVertScalp, ...
                  'jTextVertOuter',   jTextVertOuter, ...
                  'jTextVertInner',   jTextVertInner, ...
...
function s = GetPanelContents() %#ok<DEFNU>
    ...
    % Bem layers
    nvert1 = str2double(char(ctrl.jTextVertScalp.getText()));
    nvert2 = str2double(char(ctrl.jTextVertOuter.getText()));
    nvert3 = str2double(char(ctrl.jTextVertInner.getText()));
