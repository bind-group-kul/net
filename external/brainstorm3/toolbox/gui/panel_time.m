...
function UpdatePanel(varargin)
    ...
    if ~isempty(GlobalData.UserTimeWindow.Time)
        ...
        if isRaw 
            ...
            strSmp = sprintf('   %d samples', GlobalData.UserTimeWindow.NumberOfSamples);
        end
        % Format strings
        ...
        strTime = sprintf([...
            '<HTML><TABLE><TR><TD>  Time:   <B>' strTimeWindow '</B> ' timeUnit '</TD></TR>' ...
            '<TR><TD>  Sampling: %d Hz ' strSmp '</TD></TR></TABLE>'], ...
            round(1 / GlobalData.UserTimeWindow.SamplingRate));
