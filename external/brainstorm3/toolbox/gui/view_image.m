...
figurePos(1:2) = round((scrSize(3:4) - figurePos(3:4)).*[.3 .7]);
...
hMenu = uimenu('Label', 'File');
    uimenu(hMenu, 'Label', 'Save as...', 'Accelerator', 's', 'Callback', @menuFileSaveAs_Callback);
...
isWindowInitialized = 1;
    ...
    % --------------------------------------------------------------------- 
    % Menu : File->Save as...
    function menuFileSaveAs_Callback(hObject, ev)       
        % === Ask user filename ===
        % Open a dialog box to save the current figure
        [fName, fPath] = uiputfile( ...
            {'*.tif;*.jpg;*.bmp;*.png;*.hdf;*.pbm;*.pgm;*.ppm','All supported image files';
            ...
            'Save image as...', defaultFile);

        % Tests if a file was selected
        if isequal(fName,0)
            return
        end
        % Save image
        savedImg = uint8(double(img) / double(max(img(:))) * 255);
        out_image(bst_fullfile(fPath, fName), savedImg);
        
        % Save new default export path
        LastUsedDirs = bst_get('LastUsedDirs');
        LastUsedDirs.ExportImage = fPath;
    ...
    function imageMouseWheel_Callback(hObject, ev)
        ...
        elseif (ev.VerticalScrollCount < 0)
            % ZOOM IN
            Factor = 1 - ev.VerticalScrollCount ./ 20;
        elseif (ev.VerticalScrollCount > 0)
            % ZOOM OUT
            Factor = 1./(1 + ev.VerticalScrollCount ./ 20);
        end
