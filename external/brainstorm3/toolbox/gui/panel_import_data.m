...
%
% USAGE: [bstPanelNew, panelName] = panel_import_data('CreatePanel', sFile)
...
%% ===== CREATE PANEL =====
function [bstPanelNew, panelName] = CreatePanel(sFile) %#ok<DEFNU>
    ...
    isSplitPanel = ~isEpochs && (nSamples > 2000);
    ...
    % ===== SENSORS DEFINITION =====
    jPanelSensors = gui_river([0 3], [0 13 10 0], 'MEG sensors');
        % === WARNING ===
        ...
        isSspCheck      = isfield(sFile.channelmat, 'Projector') && ~isempty(sFile.channelmat.Projector);
        isCtfRecordings = ~isempty(sFile.prop.currCtfComp);
        % If it is CTF recordings: display current compensation level
        if isCtfRecordings
            jPanelSensors.add(JLabel(sprintf('Current compensation: %d', sFile.prop.currCtfComp)));
        end
        % === CTF COMPENSATORS ===
        ...
        % SSP Checkbox
        if isSspCheck
            nActive = nnz([sFile.channelmat.Projector.Status] >= 1);
            nInactive = length(sFile.channelmat.Projector) - nActive;
            nTotal = 0;
            for iProj = 1:length(sFile.channelmat.Projector)
                if (sFile.channelmat.Projector(iProj).Status >= 1)
                    if ~isempty(sFile.channelmat.Projector(iProj).CompMask)
                        nTotal = nTotal + nnz(sFile.channelmat.Projector(iProj).CompMask);
                    else
                        nTotal = nTotal + 1;
                    end
                end
            end
            jCheckSsp = gui_component('checkbox', jPanelSensors, 'br', 'Apply Signal Space Projections (SSP)', [], [], [], []);
            gui_component('label', jPanelSensors, 'br', sprintf('<HTML>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Categories: %d active/%d inactive   -   Total projectors: <B>%d</B>', nActive, nInactive, nTotal), [], [], [], []);
        else
            jCheckSsp = [];
        end
    ...
    jPanelDatabase = gui_river([0 0], [0 13 5 0], 'Database');       
        % === CREATE CONDITIONS ===
        jCheckCreateCond = JCheckBox('Create new conditions for epochs', 1);
        ...
        for i = 1:nbEvents
            ...
            if ~isempty(iSelSmp)
                ...
                if isExtended(i)
                    ...
                    if ~isempty(strfind(lower(lab), 'bad'))
                        strTag = '[bad]';
                    else
                        strTag = '[ext]';
                    end
