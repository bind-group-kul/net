function panelContents = gui_show_dialog(panelTitle, fcnPanel, isModal, figPos, varargin)
% GUI_SHOW_DIALOG: Display a BstPanel in a JDialog.
% Wait for the end of its execution, and return the panel contents.
%
% USAGE:  panelContents = gui_show_dialog(panelTitle, fcnPanel, isModal, figPos, varargin)
% 
% INPUT:
%     - panelTitle : Title of the JDialog
%     - fcnPanel   : handle to a panel_...() function
%     - isModal    : boolean
%     - figPos     : [x, y] or [x, y, width, height]
...
% Create panel
isAlwaysOnTop = 1;
Container = gui_show(bstPanel, 'JavaWindow', panelTitle, [], isModal, isAlwaysOnTop, 0);
% Set position
if (length(figPos) >= 2)
    jBstFrame = bst_get('BstFrame');
    if ~isempty(jBstFrame)
        figPos(1) = figPos(1) + jBstFrame.getLocation().getX();
        figPos(2) = figPos(2) + jBstFrame.getLocation().getY();
    end
    Container.handle{1}.setLocation(figPos(1),figPos(2));
end
% Set size
if (length(figPos) >= 4)
    error('Todo');
