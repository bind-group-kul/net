...
function bstPanelNew = CreatePanel() %#ok<DEFNU>
    ...
    jPanelFilter.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createEmptyBorder(7,7,7,7), jBorder));
        ...
        % === SIN REMOVAL ===
        jCheckSinRem = gui_component('checkbox', jPanelFilter, 'br', 'Sin. removal:', [], [], @CheckSinRemoval_Callback);
...
function UpdatePanel()
    ...
    % Change tab color
    if (Filters.HighPassEnabled || Filters.LowPassEnabled || Filters.LowPassEnabled)
        color = [.7 0 0];
    else
        color = [0 0 0];
    end
