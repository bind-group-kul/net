...
%        nodeFound = panel_protocols('SelectNode',          noreRoot, nodeType, iStudy, iFile )
%        nodeFound = panel_protocols('SelectNode',          noreRoot, FileName )
...
%      OutputFiles = panel_protocols('TreeInverse',         bstNodes)
...
function ExpandPath(bstNode, isSelected)
    ...
    if (path_.getPathCount() < 2)
        return
    end
    % Set path selected
    if isSelected
        ctrl.jTreeProtocols.setSelectionPath(path_);
    end
    % Expand path
    ctrl.jTreeProtocols.expandPath(path_);
    % Scroll to visible
    ctrl.jTreeProtocols.scrollPathToVisible(path_);
end
...
function nodeFound = GetNode( nodeRoot, nodeTypes, iStudy, iFile )
    ...
    if isExpand
        ...
        for i = 1:length(nodeStudy.getChildCount())
            ...
            if strcmpi(nodeChild.getType(), 'datalist')
...
function isCopied = CopyNode( bstNodes, isCut )
    ...
    if ismember(nodeType{1}, {'channel', 'anatomy', 'noisecov'}) && (length(bstNodes) > 1)
    ...
    elseif any(ismember(nodeType, {'link', 'rawdata'}))
        ...
        return;
    % No stat
    elseif any(ismember(nodeType, {'pdata', 'presults', 'ptimefreq'}))
    ...
    elseif any(ismember(nodeType, {'timefreq', 'spectrum'})) && ~isempty(strfind(char(bstNodes(1).getFileName()), '_KERNEL_'))
        bst_error('This type of file cannot be copied or moved.', 'Clipboard', 0);
        return;
    % Datalist: copy all the child nodes
    elseif strcmpi(nodeType, 'datalist')
        listNodes = bstNodes;
...
function destFile = PasteNode( targetNode )
    ...
    elseif (iTarget == 0)
        ...
        sSubjectTargetRaw = bst_get('Subject', sStudyTarget.BrainStormSubject, 1);
    end
    % Channel/Headmodel/NoiseCov/Kernel: Make sure that target study is the right one
    if ismember(firstSrcType, {'channel', 'headmodel', 'noisecov', 'kernel'})
    ...
    for i = 1:length(srcNodes)
        ...
        if ~isAnatomy
            ...
            isSameSubj = file_compare(sSubjectTargetRaw.FileName, sStudySrc.BrainStormSubject);
        else
            isSameSubj = (iSrcStudy == iTarget);
        end
        % Cannot copy (channel/noisecov/MRI) or move to the same folder
        if (isCut || ismember(srcType, {'channel', 'noisecov', 'anatomy'})) && (iSrcStudy == iTarget)
        ...
        if strcmpi(srcType, 'noisecov')
            destFile = cat(2, destFile, db_set_noisecov(iSrcStudy, iTarget));
            continue;
        end
        ...
        elseif ismember(srcType, {'results', 'headmodel', 'timefreq', 'spectrum', 'presults', 'ptimefreq'})
            src = load(file_fullpath(srcFile));
            if (iSrcStudy ~= iTarget)
                if isfield(src, 'DataFile')
                    src.DataFile = '';
                end
                if isfield(src, 'HeadModelFile') && ~isSameSubj
                    src.HeadModelFile = '';
                end
                if isfield(src, 'SurfaceFile') && ~isSameSubj
                    src.SurfaceFile = '';
                end
            end
        else
        ...
        % Copy file
        destFile{i} = db_add(iTarget, src, 0);
        ...
        if ismember(srcType, {'cortex','scalp','innerskull','outerskull','other'})
            [tmpTessMat, TessFile] = in_tess_bst(destFile{i}, 0);
...
%% ===== NODE: INVERSE MODEL =====
function OutputFiles = TreeInverse(bstNodes) %#ok<DEFNU>
    OutputFiles = {};
    % Get node type
    nodeType = lower(char(bstNodes(1).getType()));
    % Data file: gets "data file" dependence, else get channel studies
    if strcmpi(nodeType, 'data')
        [iStudies, iDatas] = tree_dependencies(bstNodes, nodeType);
    else
    ...
    [OutputFiles, errMessage] = panel_inverse('ComputeInverse', iStudies, iDatas);
    % Error
    if isempty(OutputFiles) && ~isempty(errMessage)
