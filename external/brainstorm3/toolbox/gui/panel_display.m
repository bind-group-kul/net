...
function bstPanelNew = CreatePanel() %#ok<DEFNU>
    ...
    jPanelSelect.setVisible(0);
        ...
        % Checkbox: Hide edge effects
        jCheckHideEdge = gui_component('Checkbox', jPanelSelect, 'br', 'Hide edge effects', [], '', @HideEdge_Callback);
    ...
    jPanelFunction.setVisible(0);
        ...
        jRadioFunPower = gui_component('Radio', jPanelFunction, 'br', 'Power',      jButtonGroup, '', @(h,ev)TfFunction_Callback('power'));
        jRadioFunMag   = gui_component('Radio', jPanelFunction, 'br', 'Magnitude',  jButtonGroup, '', @(h,ev)TfFunction_Callback('magnitude'));
        jRadioFunLog   = gui_component('Radio', jPanelFunction, 'br', 'Log(power)', jButtonGroup, '', @(h,ev)TfFunction_Callback('log'));
        jRadioFunPhase = gui_component('Radio', jPanelFunction, 'br', 'Phase',      jButtonGroup, '', @(h,ev)TfFunction_Callback('phase'));
    ...
    jPanelPac.setVisible(0);
        % Radio: Select function to apply on top of the TF values
        jButtonGroup = ButtonGroup();
        jRadioPacMax   = gui_component('Radio', jPanelPac, 'br', 'MaxPAC',    jButtonGroup, '', @PacValue_Callback);
        jRadioPacFlow  = gui_component('Radio', jPanelPac, 'br', 'Freq low',  jButtonGroup, '', @PacValue_Callback);
        jRadioPacFhigh = gui_component('Radio', jPanelPac, 'br', 'Freq high', jButtonGroup, '', @PacValue_Callback);
        jRadioPacPhase = gui_component('Radio', jPanelPac, 'br', 'PhasePAC',  jButtonGroup, '', @PacValue_Callback);
...
%  =================================================================================            
    ...
    function TfFunction_Callback(target)
        % Update display options
        SetDisplayOptions();
    end

    %% ===== RADIO BUTTON: CHANGE PAC VALUE =====
    function PacValue_Callback(h, ev)
        % Update display options
        SetDisplayOptions();
    end

    %% ===== CHECKBOX: HIDE EDGES =====
    function HideEdge_Callback(h, ev)
        % Update display options
        SetDisplayOptions();
    end
...
function UpdatePanel(hFig)
    ...
    switch lower(TfInfo.Function)
        ...
        case 'pacphase',  ctrl.jRadioPacPhase.setSelected(1);
    end
    % Entire panel
    if ~ismember(lower(TfInfo.Function), {'maxpac', 'pacflow', 'pacfhigh', 'pacphase'}) || ~isequal(GlobalData.DataSet(iDS).Timefreq(iTimefreq).Freqs, 0)
        ctrl.jPanelPac.setVisible(0);
    else
    ...
    AllRowNames = GlobalData.DataSet(iDS).Timefreq(iTimefreq).RowNames;
    % Update row list
    if ismember(TfInfo.DisplayMode, {'AllSensors', '2DLayout', '2DLayoutOpt'}) || isempty(TfInfo.RowName) || (iscell(TfInfo.RowName) && (length(TfInfo.RowName) > 1))
        isEnabledRow = UpdateRowList({}, []);
    elseif iscell(AllRowNames)
        isEnabledRow = UpdateRowList(AllRowNames, TfInfo.RowName);
    else
...
function sOptions = GetDisplayOptions()
    ...
    elseif ctrl.jRadioPacPhase.isSelected()
        sOptions.Function = 'pacphase';
    else
...
function SetDisplayOptions(sOptions)
    ...
    if isequal(TfInfo.Function, sOptions.Function) && isequal(TfInfo.HideEdgeEffects, sOptions.HideEdgeEffects) && ...
    ...
    switch (GlobalData.DataSet(iDS).Figure(iFig).Id.Type)
        ...
        case 'Pac',        figure_pac('UpdateFigurePlot', hFig);
        case 'Timefreq'
            % Update this figure
            figure_timefreq('UpdateFigurePlot', hFig, 1);
            % If there is a selected RowName and if it was updated: Try updating other figures
            if ~isempty(sOptions.RowName) && ~isequal(sOptions.RowName, prevRowName)
