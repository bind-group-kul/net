...
if ~isdir(OpenmeegDir) || isempty(dir(bst_fullfile(OpenmeegDir, 'om_gain*'))) || ~strcmpi(prevUrl, url) || isUpdate
    ...
    if isempty(tgzFile)
        ...
        downloadManager = java_create('org.brainstorm.file.BstDownload', 'Ljava.lang.String;Ljava.lang.String;Ljava.lang.String;)', url, tgzFile, 'OpenMEEG update');
        ...
        if (downloadManager.getResult() ~= 1)
            errMsg = ['Impossible to download OpenMEEG:' 10 ...
                      downloadManager.getMessage()];
            return
        end
    ...
    % Delete files
    file_delete({tgzFile, unzipDir}, 1);
...
if isempty(hminvfile) && isempty(hmfile)
    ...
    % === BUILD HM FILE ===
    % Build HM file
    if ~bst_system(['om_assemble -HM "' geomfile '" "' condfile '" "' hmfile '"'], 'Assembling head matrix...')
        return
    end
    % === INVERSE HM ===
    if ~OPTIONS.isAdjoint
        if ~bst_system(['om_minverser "' hmfile '" "' hminvfile '"'], 'Inverting head matrix...');
            return
        end
    end
...
if ~OPTIONS.isAdjoint
    ...
    if ~bst_system(['om_assemble ' strDsm ' "' geomfile '" "' condfile '" "' dipfile '" "' dsmfile '"'], 'Assembling dipoles source matrix...')
        return
    end
end
...
if isEeg
    ...
    % Compute HE2EM
    if ~bst_system(['om_assemble -H2EM "' geomfile '" "' condfile '" "' eegloc_file '" "' h2emfile '"'], 'Assembling EEG interpolation operator...')
        return
    end
end
% === MEG ===
if isMeg
    ...
    fclose(fid);
    % Compute H2MM
    if ~bst_system(['om_assemble -H2MM "' geomfile '" "' condfile '" "' megloc_file '" "' h2mmfile '"'], 'Assembling MEG interpolation operator...')
        return
    end
end
% === ECOG ===
if isEcog
    ...
    save(ecogloc_file, 'ecogloc', '-ASCII', '-double', '-v6');
    % Compute H2ECOGM
    if ~bst_system(['om_assemble -H2ECOGM "' geomfile '" "' condfile '" "' ecogloc_file '" "' h2ecogmfile '"'], 'Assembling ECOG interpolation operator...')
        return
    end
end
% === SEEG ===
if isSeeg
    ...
    save(seegloc_file, 'seegloc', '-ASCII', '-double', '-v6');
    % Compute H2IPM 
    if ~bst_system(['om_assemble -H2IPM "' geomfile '" "' condfile '" "' seegloc_file '" "' h2ipmfile '"'], 'Assembling SEEG interpolation operator...')
        return
    end
end
...
if isEeg
    ...
    if  ~OPTIONS.isAdjoint
        res = bst_system(['om_gain -EEG "' hminvfile '" "' dsmfile '" "' h2emfile '" "' eeggain_file '"'], 'Assembling EEG leadfield...');
    else
        res = bst_system(['om_gain -EEGadjoint "' geomfile '" "' condfile '" "' dipfile '" "' hmfile '" "' h2emfile '" "' eeggain_file '"'], 'Assembling EEG leadfield...');
    end
    if ~res            
        return
    end
...
if isMeg
    % Compute DS2MM
    if ~bst_system(['om_assemble -DS2MM "' dipfile '" "' megloc_file '" "' ds2megfile '"'], 'Assembling MEG dipoles source matrix...')
        return
    end
    % Compute MEG leadfield
    if OPTIONS.isAdjoint
        res = bst_system(['om_gain -MEGadjoint "' geomfile '" "' condfile '" "' dipfile '" "' hmfile '" "' h2mmfile '" "' ds2megfile '" "' meggain_file '"'], 'Assembling MEG leadfield...');
    else
        res = bst_system(['om_gain -MEG "' hminvfile '" "' dsmfile '" "' h2mmfile '" "' ds2megfile '" "' meggain_file '"'], 'Assembling MEG leadfield...');
    end
    if ~res
        return
    end
...
if isEcog
    ...
    res = bst_system(['om_gain -EEG "' hminvfile '" "' dsmfile '" "' h2ecogmfile '" "' ecoggain_file '"'], 'Assembling ECOG leadfield...');
    if ~res            
        return
    end
    % Read ECOG leadfield
...
if isSeeg
    % Compute DS2IPM
    if ~bst_system(['om_assemble -DS2IPM "' geomfile '" "' condfile '" "' dipfile '" "' seegloc_file '" "' ds2ipmfile '"'], 'Assembling SEEG dipoles source matrix...')
        return
    end
    % Compute SEEG leadfield
    res = bst_system(['om_gain -IP "' hminvfile '" "' dsmfile '" "' h2ipmfile '" "' ds2ipmfile '" "' seeggain_file '"'], 'Assembling SEEG leadfield...');
    if ~res
        return
    end
    % Read SEEG leadfield
...
%% ===== SYSTEM CALL =====
    function isOk = bst_system(strcall, strProgress)
        % Progress bar
        ...
        if (status ~= 0)
            ...
            errMsg = ['OpenMEEG call: ',  strrep(strcall, ' "', [10 '      "']),  10 10 ...
                      'OpenMEEG error #' num2str(status) ': ' 10 result];
