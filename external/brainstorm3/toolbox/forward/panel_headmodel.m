...
%% ===== CREATE PANEL =====
function [bstPanelNew, panelName] = CreatePanel(isMeg, isEeg, isEcog, isSeeg)  %#ok<DEFNU>
    ...
    jRadioGridVolume  = gui_component('Radio', jPanelSourceSpace, 'br hfill', 'MRI volume [Experimental]', jButtonGroupGridType,  'Estimates sources in the full head volume', @UpdateComment);
    % Default: surface
...
%  =================================================================================
    ...
    function UpdateComment(varargin)
        ...
        % Grid type
        isVolumic = jRadioGridVolume.isSelected();
        if isVolumic
...
function [OutputFiles, errMessage] = ComputeHeadModel(iStudies, sMethod) %#ok<DEFNU>
    ...
    if (nargin < 2) || isempty(sMethod)
        sMethod = gui_show_dialog('Compute head model', @panel_headmodel, 1, [], isMeg, isEeg, isEcog, isSeeg);
        if isempty(sMethod)
            return;
        end
    end
