...
%     .Projector  : SSP vectors applied to the data
%
%     ======= METHOD SELECTION ============================================
...
% Free memory
clear Def_OPTIONS DefFieldNames
...
switch (OPTIONS.HeadModelType)
    ...
    case 'dba'
        % do nothing : GridLoc & GridOrient are already stored in OPTIONS
end
...
if ~isempty(allLoc)
    ...
    if ~isempty(iVertInside)
        errMessage = ['Some EEG or MEG sensors are indicated to be located inside the brain volume.' 10 'You should check the position of the electrodes.'];
    end
end
...
if ismember('openmeeg', {OPTIONS.MEGMethod, OPTIONS.EEGMethod, OPTIONS.ECOGMethod, OPTIONS.SEEGMethod})
    ...
    for iLayer = 1:length(OPTIONS.BemNames)
        strHistory = [strHistory, ' | ', sprintf('%s %1.4f', OPTIONS.BemNames{iLayer}, OPTIONS.BemCond(iLayer)) ];
    end
else
    % Initializations
    Gain = NaN * zeros(length(OPTIONS.Channel), Dims * nv);
...
if ~isempty(OPTIONS.Projector)
    ...
    Projector = process_ssp('BuildProjector', OPTIONS.Projector, [1 2]);
    % Apply projectors
