function x = bst_bandpass(x, Fs, HighPass, LowPass, isFir2, isMirror, stopBand)
% BST_BANDPASS: Bandpass filter for the signal x. 
%
% USAGE:  x = bst_bandpass(x, Fs, HighPass, LowPass, isFir2, isMirror)
%         x = bst_bandpass(x, Fs, HighPass, LowPass)         % isFir2 = 1, isMirror = 1
%
% INPUT: 
...
%    Hish-stop filter: HighStop = HighPass - min(5, HighPass / 2);
%
...
% Else: cut unwanted frequencies (FFT mask)
    ...
    if ~isempty(LowPass) && (LowPass ~= 0)
        iLowStop = min(round(LowStop / Fs * N), N/2);
        iLowPass = min(round(LowPass / Fs * N), N/2);
        H(iLowStop:end) = 0;
