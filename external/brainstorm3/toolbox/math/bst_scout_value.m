function Fs = bst_scout_value(F, ScoutFunction, Orient, nComp, XyzFunction, isSignFlip)
% BST_SCOUT_VALUE: Combine Ns time series using the given function. Used to get scouts/clusters values.
%
% USAGE:  Fs = bst_scout_value(F, ScoutFunction, Orient=[], nComp=1, XyzFunction='none', isSignFlip=0)
%         Fs = bst_scout_value(F, ScoutFunction)
%
% INPUTS:
%     - F              : [Nsources * Ncomponents, Ntime] double matrix, source time series
%     - ScoutFunction  : String, function to use to combine the Nsources time series {'mean', 'mean_norm', 'max', 'power', 'pca', 'fastpca', 'stat', 'all', 'none'}
%     - Orient         : [Nsources x 3], Orientation of each source - usually the normal at the vertex in the cortex mesh
%     - nComp          : {1,2,3}, Number of components per vertex in matrix F 
...
% =============================================================================@
%
% Authors: Sylvain Baillet, Francois Tadel, John Mosher, 2010-2012
...
% No function to apply at all: return initial data
...
if isSignFlip && (nComp == 1) && ~isempty(Orient) && (size(F,1) > 1) && ~all(F(:) > 0)
