...
srcPts_mr  = cs_scs2mri(sMriSrc, srcPts'  * 1000)';
destPts_mr = cs_scs2mri(sMriSrc, destPts' * 1000)';
% Divide by voxel size to get MRI coordinates in voxels
srcPts_mr  = bst_bsxfun(@rdivide, srcPts_mr,  sMriSrc.Voxsize);
destPts_mr = bst_bsxfun(@rdivide, destPts_mr, sMriSrc.Voxsize);
...
% sMriDest.SCS  = destSCS;
sMriDest.NCS  = destNCS;
% % SCS: Create transformation matrix (MRI <-> SCS)
% scsTransf = cs_mri2scs(sMriDest);
% if isempty(scsTransf)
%     sMriDest = rmfield(sMriDest, 'SCS');
% else
%     sMriDest.SCS.R      = scsTransf.R;
%     sMriDest.SCS.T      = scsTransf.T;
%     sMriDest.SCS.Origin = scsTransf.Origin;
% end
% MNI: Create transformation matrix (MRI <-> MNI)
mniTransf = cs_mri2mni(sMriDest);
if isempty(mniTransf)
    sMriDest = rmfield(sMriDest, 'NCS');
else
    sMriDest.NCS.R      = mniTransf.R;
    sMriDest.NCS.T      = mniTransf.T;
    sMriDest.NCS.Origin = mniTransf.Origin;
end
