...
if isempty(sColin)
    bst_error(['The template anatomy Colin27 is not available.' 10 ...
               'Please update Brainstorm...'], 'BEM surfaces', 0);
    return
end
% Get subject file
ColinCortexFile = bst_fullfile(sColin.FilePath, 'tess_cortex_pial_low.mat');
ColinInnerFile  = bst_fullfile(sColin.FilePath, 'tess_innerskull.mat');
ColinMriFile    = bst_fullfile(sColin.FilePath, 'subjectimage_T1.mat');
...
sColinCortex = tess_envelope(ColinCortexFile, 'convhull', BemOptions.nvert(3), 0.001, ColinMriFile);
if isempty(sColinCortex)
    return;
end
bst_progress('inc', 15);
% === COLIN INNER SKULL ===
...
bst_progress('inc', 10);
sColinInner = in_tess_bst(ColinInnerFile, 0);
% Downsample Colin inner skull if necessary
if (length(sColinInner.Vertices) > 3000)
    ratio = 1000 ./ length(sColinInner.Vertices);
    [sColinInner.Faces, sColinInner.Vertices] = reducepatch(sColinInner.Faces, sColinInner.Vertices, ratio);
end
...
vColinCortex = bst_bsxfun(@minus, sColinCortex.Vertices, sColinCortex.center) * sColinCortex.R;
vColinInner  = bst_bsxfun(@minus, sColinInner.Vertices,  sColinCortex.center) * sColinCortex.R;
...
rColinCortex = tess_parametrize_new(vColinCortex, th, phi);
...
[rMax_c,iPhiMax_c] = max(rColinCortex(:,iThMax));
[vMax_c(1),vMax_c(2),vMax_c(3)] = sph2cart(th(iThMax), phi(iPhiMax_c), rMax_c);
[rMin_c,iPhiMin_c] = max(rColinCortex(:,iThMin));
...
R_c = [cos(ay) 0 -sin(ay);
          0    1    0    ;
       sin(ay) 0  cos(ay)];
% Reorient Colin surfaces
vColinCortex = vColinCortex * R_c';
vColinInner  = vColinInner * R_c';
vMin_c = vMin_c * R_c';
vMax_c = vMax_c * R_c';
...
minv = min(vCortex);
maxv = max(vCortex);
minv_c = min(vColinCortex);
maxv_c = max(vColinCortex);
scale = (maxv-minv) ./ (maxv_c-minv_c);
offset = minv - minv_c .* scale;
% Force the Colin surfaces to fit into the subject box
vColinCortex = bst_bsxfun(@times, vColinCortex, scale);
vColinCortex = bst_bsxfun(@plus,  vColinCortex, offset);
vColinInner  = bst_bsxfun(@times, vColinInner, scale);
vColinInner  = bst_bsxfun(@plus,  vColinInner, offset);
...
if DEBUG
    ...
    [hFig, iDS, iFig, hPatch] = view_surface_matrix(vColinCortex, sColinCortex.Faces, .7, [0 1 0], hFig);
...
rCortex      = tess_parametrize_new(vCortex,      th, phi);
rColinCortex = tess_parametrize_new(vColinCortex, th, phi);
rColinInner  = tess_parametrize_new(vColinInner,  th, phi);
...
cortex2inner = rColinInner ./ rColinCortex;
...
BemHeadFile  = file_unique(bst_fullfile(SurfaceDir, 'tess_head_bem.mat'));
BemOuterFile = file_unique(bst_fullfile(SurfaceDir, 'tess_outerskull_bem.mat'));
BemInnerFile = file_unique(bst_fullfile(SurfaceDir, 'tess_innerskull_bem.mat'));
% Save head
